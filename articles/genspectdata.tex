...
Process declarations do not allow pattern matching of the left hand side of the
equation. However, we can use pattern matching indirectly through the use of
summations in the right hand side. As an example, take the following process
declaration without pattern matching, where \frm{T} stands for \frm{\f{BATree}}:
\begin{genspect}
proc & \fa{P}{t: T} & = &
    \fa{is\_leaf}{t} & \To
...
\noindent
Note that is has to be investigated whether the current tools are able to
handle this in the right way. If this is not the case, this should be added.
