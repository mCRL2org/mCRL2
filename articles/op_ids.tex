...
declaration. It is called system defined if it is defined in this document.

System identifiers are created during the parsing phase and the data removal
phase. After the parsing phase the types of system identifiers are not known;
these are determined in the type check phase. More precisely, after parsing
system identifiers are available as \f{DataVarOpId}'s. During the type checking
phase these system identifiers are translated to \f{OpId}'s with the correct
type. System identifiers created in the data removal phase are not yet treated.
...
identifiers for booleans, numbers, lists, sets and bags.
...
\verb+==+    & \srtbool \times \srtbool \To \srtbool & equality     & infix\\
\verb+!=+    & \srtbool \times \srtbool \To \srtbool & inequality   & infix\\
...
\srtnat, \srtint}} and \frm{C} is the bigger type of \frm{A} and \frm{B}:
...
\verb-+-    & A \times B \To C             & addition             & infix\\
\verb+-+    & A \times B \To \srtint       & subtraction          & infix\\
\verb+*+    & A \times B \To C             & multiplication       & infix\\
\verb+div+  & \srtpos \times \srtpos \To \srtnat & quotient after division
                                                                  & infix\\
...
\verb+div+  & \srtint \times \srtpos \To \srtint & quotient after division
                                                                  & infix\\
\verb+mod+  & A \times \srtpos \To \srtnat & remainder after division
                                                                  & infix\\
\verb+exp+  & \srtpos \times \srtnat \To \srtnat & exponentation  & functional\\
\verb+exp+  & \srtnat \times \srtnat \To \srtnat & exponentation  & functional\\
\verb+exp+  & \srtint \times \srtnat \To \srtint & exponentation  & functional\\
...
\verb+abs+  & \srtpos \To \srtpos          & absolute value       & functional\\
\verb+abs+  & \srtnat \To \srtnat          & absolute value       & functional\\
\verb+abs+  & \srtint \To \srtnat          & absolute value       & functional\\
\verb+max+  & A \times B \To C             & maximum              & functional\\
\verb+min+  & A \times B \To C             & minimum              & functional\\
\verb-<-    & A \times B \To \srtbool      & less than            & infix\\
\verb->-    & A \times B \To \srtbool      & greater than         & infix\\
\verb+<=+   & A \times B \To \srtbool      & less than or equal   & infix\\
\verb+>=+   & A \times B \To \srtbool      & greater than or equal& infix\\
