...
listed in table~\ref{tab:ptc:numbers}. Here, we assume $A,B \in \set{\srtpos,
\srtnat, \srtint}$ with $A \neq B$. $A_{s}$ restricts $A$ to those sorts that
have a prefix in the string $s$, e.g.\ $A_{\f{PN}} \in \set{srtpos, srtnat}$.
Also for numeric sorts $X,Y$, $X \cap Y$ represent the intersection of the
underlying data types, i.e.\ $\llbracket X \cap Y\rrbracket = \llbracket X
\rrbracket \cap \llbracket Y \rrbracket$. Note that there always exists a
unique sort for such an expression.
...
\verb+min+        & A \times B \To A \cup B& minimum              & functional\\
...
\verb-+-          & A_{\f{PN}} \times B_{\f{PN}} \To A_{\f{PN}} \cap B_{\f{PN}}
                                           & addition             & infix\\
\verb-+-          & A \times \srtint \To \srtint
                                           & addition             & infix\\
\verb-+-          & \srtint \times A \To \srtint
                                           & addition             & infix\\
