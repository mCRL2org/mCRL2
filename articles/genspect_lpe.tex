...
\aterm{True}() & & \Bool 
\aterm{False}() & & \Bool 
\aterm{EmptyList}() & & \List(\seq{}) 
\aterm{EmptySetBag}() & & \SB(\seq{})
\aterm{ListEnum}(<DataExpr>+) &\text{argument types are compatible} & \List(Copmat(args))
\aterm{SetEnum}(<DataExpr>+) & \text{similar}&
\aterm{BagEnum}(BagEnumElt+) & \text{..} &
\aterm{SetBagComp}(<IdDecl>, <DataExpr>) & type(arg2)=\Bool, arg2 in arg1 & \SB{type(target arg1)}
\aterm{FuncApp}(<DataExpr>, <DataExpr>*) & types(arg2) are compat source(arg1), & target(arg1) 
\aterm{NotOrCompl}(<DataExpr>) & &
\aterm{Neg}(<DataExpr>) & &
\aterm{Size}(<DataExpr>) 
\aterm{Forall}(<IdDecl>, <DataExpr>) 
\aterm{Exists}(<IdDecl>, <DataExpr>) 
\aterm{ListAt}(<DataExpr>, <DataExpr>) 
\aterm{MultOrIntersect}(<DataExpr>, <DataExpr>) 
\aterm{Div}(<DataExpr>, <DataExpr>) 
\aterm{Mod}(<DataExpr>, <DataExpr>) 
\aterm{AddOrUnion}(<DataExpr>, <DataExpr>) 
\aterm{SubtOrDiff}(<DataExpr>, <DataExpr>) 
\aterm{LTOrPropSubset}(<DataExpr>, <DataExpr>) 
\aterm{GT}(<DataExpr>, <DataExpr>) 
\aterm{LTEOrSubset}(<DataExpr>, <DataExpr>) 
\aterm{GTE}(<DataExpr>, <DataExpr>) 
\aterm{In}(<DataExpr>, <DataExpr>) 
\aterm{Cons}(<DataExpr>, <DataExpr>) 
\aterm{Snoc}(<DataExpr>, <DataExpr>) 
\aterm{Concat}(<DataExpr>, <DataExpr>) 
\aterm{Eq}(<DataExpr>, <DataExpr>) 
\aterm{Neq}(<DataExpr>, <DataExpr>) 
\aterm{And}(<DataExpr>, <DataExpr>) 
\aterm{Or}(<DataExpr>, <DataExpr>) 
\aterm{Imp}(<DataExpr>, <DataExpr>) 
\aterm{Lambda}(<IdDecl>, <DataExpr>) 
\aterm{Whr}(<DataExpr>, <DataExpr>+) 
