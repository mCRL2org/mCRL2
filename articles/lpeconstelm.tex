...
$\varphi$ is a function of $\ovr{D} \times \cup_{Nat} \rightarrow {D}$:
\begin{displaymath}
    \varphi(\ovr{d},X) = \left \{
        \begin{array}{l l}
            X = \emptyset                       & \rightarrow \lbrace \ovr{d} \rbrace \\
            X \neq \emptyset \wedge z \in S     & \rightarrow \cup_{y \in \ovr{D}._z} \varphi(\ovr{d}@(y), S\setminus \lbrace z \rbrace ) \\
        \end{array} \right.
\end{displaymath}
...
\tmp $ j \in \lbrace k \in \lbrace 1, \ldots, n \rbrace \vert \forall_{i \in I, \ovr{e_i} \in \ovr{E_i}}(\forall_{\ovr{x} \in \lbrace
...
\tmp $ \equiv$ \hspace{20pt} $\lbrace y._k = \ovr{x}._k  \rbrace$\\
...
\> \> \> $ c_i\z \neq false \Rightarrow $ \\
\> \> \> \> $(\ovr{v_0}._j \not\in FV \rightarrow {\ovr{g_i}\z._j \not\in FV} \Rightarrow ( \ovr{g_i}\z._j = \ovr{v_0}._j ))$\\
\> \> \> $\wedge$\\
\> \> \> \> $(\ovr{v_0}._j \in FV \rightarrow \exists_k(\ovr{g_i}\z._j \not\in FV   \Rightarrow (\ovr{g_i}\z._j = k))) $\\
% \> \> \> $)$ \\
\> \>$)$\\
\> $)$ \\
% $)$
