...
\begin{displaymath}
    \varphi(\ovr{d},X) = \left \{
        \begin{array}{l l}
            ...
            X \neq \emptyset                    & \rightarrow \cup_{y \in \ovr{D}._z} \varphi(\ovr{d}@(y), X \setminus \lbrace z \rbrace ) \hspace{20pt} \text{with } z \in X \\
        \end{array} \right.
\end{displaymath}
...
\tmp $ j \in \lbrace k \in \lbrace 1, \ldots, n \rbrace \vert \forall_{i \in I, \ovr{e_i} \in \ovr{E_i}}(\forall_{\ovr{x} \in
 ...
 ) \rbrace}(c_i(\ovr{x, e_i}) \Rightarrow \ovr{v_0}._k = \ovr{g_i}(\ovr{x, e_i})._k )) \rbrace$\\
...
\newcommand{\z}{(\otimes_{S_L^4}^{FV}(\ovr{v_0}),e_i)}
...
\> \> \> $ c_i\z \neq false \Rightarrow $ \\
\> \> \> \> $(NF(\ovr{v_0}._j) \not\in FV \rightarrow {NF(\ovr{g_i}\z._j) \not\in FV} \Rightarrow ( NF(\ovr{g_i}\z._j) = NF(\ovr{v_0}._j )))$\\
\> \> \> $\wedge$\\
\> \> \> \> $(NF(\ovr{v_0}._j) \in FV \rightarrow \exists_k( NF(\ovr{g_i}\z._j) \not\in FV \Rightarrow ( NF(\ovr{g_i}\z._j = k)))) $\\
% \> \> \> $)$ \\
\> \>$)$\\
\> $)$ \\
% $)$
...
If we have a situation simular to Example \ref{freevarcheckup}, we see that the algorithm detects the $3^{th}$ element as a constant \pp. However the $3^{th}$ is not a constant \pp . To overcome the detection of fake \pps , we have to take an extra pass on the LPE. This pass is called the "Free Variable Checkup". If the "Free Variable Checkup" finds a fake constant, the algorithm starts over with:
\begin{itemize}
