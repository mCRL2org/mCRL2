  ...
  if(MCRL2_ENABLE_TEST_HEADERS)
    ...
    foreach(header ${headers})
      ...
      if(NOT TARGET ${testname})
        add_executable(${testname} EXCLUDE_FROM_ALL "${CMAKE_SOURCE_DIR}/build/cmake/header_test.cpp")
        target_link_libraries(${testname} ${TARGET_NAME})
...
macro(_install_qt_libs TARGET_NAME COMPONENT QT_LIBS)
  if(APPLE AND QT_LIBS)
    ...
    foreach(LIB ${QT_LIBS})
      ...
      #get_filename_component(ABS_PATH ${UNRESOLVED_PATH} ABSOLUTE)
      #get_filename_component(ABS_PATH ${ABS_PATH} DIRECTORY)
      #if("${ABS_PATH}" MATCHES ".*\\.framework/*$")
        #install(DIRECTORY ${ABS_PATH} DESTINATION ${MCRL2_BUNDLE_PATH}/${TARGET_NAME}.app/Contents/Frameworks)
      #else()
      #  message(FATAL_ERROR "Could not find .framework directory for Qt${LIB} (found ${ABS_PATH})")
      #endif()
    ...
    endforeach()
    set(ABS_PATH "${MCRL2_BUNDLE_PATH}/${TARGET_NAME}.app/Contents/MacOS/${TARGET_NAME}")
...
function(_prepare_desktop_application TARGET_NAME TOOLNAME DESCRIPTION ICON)
  ...
  elseif(APPLE)# AND ("${TARGET_NAME}" STREQUAL "mcrl2-gui"))
    #set_target_properties(${TARGET_NAME} PROPERTIES
    #  MACOSX_BUNDLE TRUE
  ...
  if((NOT IS_GUI_BINARY OR MCRL2_ENABLE_GUI_TOOLS) AND ARG_COMPONENT)
    ...
    elseif(${TARGET_TYPE} STREQUAL "EXECUTABLE")
      ...
      if(IS_BUNDLE)
        install(CODE
          "execute_process(
            COMMAND \"${CMAKE_COMMAND}\" \"-E\" \"create_symlink\" \"${CMAKE_INSTALL_PREFIX}/${MCRL2_LIBRARY_PATH}\" \"${MCRL2_BUNDLE_PATH}/${TARGET_NAME}.app/Contents/lib\"
        )")
      #  install(CODE
      #    "include(BundleUtilities)
      #     set(APP \"\${CMAKE_INSTALL_PREFIX}/Applications/${TARGET_NAME}.app\")
      #     get_bundle_main_executable(\"\${APP}\" APPEXE)
      #     message(STATUS \"Main executable: \${APPEXE}\")
      #     fixup_bundle(\"\${APP}\" \"\" \"\")
      #    ")
      endif()
