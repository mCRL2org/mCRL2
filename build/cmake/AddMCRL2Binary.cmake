...
macro(_install_qt_libs TARGET_NAME COMPONENT QT_LIBS TARGET_TYPE)
  if(APPLE AND QT_LIBS)
    ...
    foreach(LIB ${QT_LIBS})
      ...
      get_filename_component(LIBNAME ${ABS_PATH} NAME)
      list(APPEND FILE_LIST "${MCRL2_LIBRARY_PATH}/${LIBNAME}")
      list(APPEND ID_LIST "@loader_path/../lib/Qt${LIB}")
    endforeach()
    if(${TARGET_TYPE} STREQUAL "LIBRARY")
      if(${CMAKE_BUILD_TYPE})
        get_target_property(LIBNAME ${TARGET_NAME} "${CMAKE_BUILD_TYPE}_LOCATION")
      else()
        get_target_property(LIBNAME ${TARGET_NAME} "LOCATION")
      endif()
      get_filename_component(LIBNAME ${LIBNAME} NAME)
      list(APPEND FILE_LIST "${MCRL2_LIBRARY_PATH}/${LIBNAME}")
    else()
      list(APPEND FILE_LIST "${MCRL2_RUNTIME_PATH}/${TARGET_NAME}")
    endif()
    ...
    install(CODE
      "
      message(STATUS \"Running ${INSTALL_NAME_TOOL} on the following files: ${FILE_LIST}\")
      set(FILE_LIST ${FILE_LIST})
      set(ID_LIST ${ID_LIST})
      foreach(FILE \${FILE_LIST})
        execute_process(COMMAND ${INSTALL_NAME_TOOL} \$ENV{DESTDIR}\${CMAKE_INSTALL_PREFIX}/\${FILE})
        if (ID_LIST)
          list(GET ID_LIST 0 ID)
          list(REMOVE_AT ID_LIST 0)
          execute_process(COMMAND install_name_tool -id \${ID} \$ENV{DESTDIR}\${CMAKE_INSTALL_PREFIX}/\${FILE})
        endif()
      endforeach()
      "
      COMPONENT ${COMPONENT})
