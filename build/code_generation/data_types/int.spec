...
eqn ==(@cInt(m),@cInt(n)) = ==(m,n);
    ...
    -(p) = @cNeg(p);
    -(@c0) = @cInt(@c0);
    -(n) = if(==(n,@most_significant_digitNat(@zero_word)),@cInt(@most_significant_digitNat(@zero_word)),@cNeg(Nat2Pos(n)));
    -(@cInt(n)) = -(n);
    -(@cNeg(p)) = @cInt(Pos2Nat(p));
    succ(@cInt(n)) = @cInt(+(n,@most_significant_digitNat(@one_word)));  
    succ(@cNeg(p)) = -(pred(p));
