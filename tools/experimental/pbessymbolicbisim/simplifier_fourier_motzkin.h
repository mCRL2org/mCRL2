...
#ifndef MCRL2_PBESSYMBOLICBISIM_SIMPLIFIER_FOURIER_MOTZKIN_H
#define MCRL2_PBESSYMBOLICBISIM_SIMPLIFIER_FOURIER_MOTZKIN_H
...
protected:
  ...
  data_expression simplify_expression(const data_expression& expr)
  {
    ...
    for(std::map< data_expression, std::vector< data_expression_list >>::iterator it = discr_to_real.begin(); it != discr_to_real.end(); it++)
    {
      ...
      result = lazy::or_(result, lazy::and_(simpl_discr.apply(down_cast<lambda>(it->first)), real_condition));
    }
    return rewr(result);
  }
