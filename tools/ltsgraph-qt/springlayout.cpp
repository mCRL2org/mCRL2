...
namespace Graph
{
  ...
  {
    Coord3D diff = (a - b);
    float dist = qMax(diff.size(), 1.0f);
  ...
  Coord3D SpringLayout::forceLinearSprings(const Coord3D& a, const Coord3D& b, float ideal)
  {
    ...
    float factor = qMax(dist, 0.0f) * m_attraction;
    // Let springs attract really strong near their equilibrium
    if (dist > 0.0f)
    {
      factor = qMax(factor, 100 * m_attraction / qMax(dist * dist / 10000.0f, 0.1f));
    }
    return diff *  factor;
  }
  ...
  Coord3D repulsionForce(const Coord3D& a, const Coord3D& b, float repulsion, float natlength)
  {
    Coord3D diff = a - b;
    float r = repulsion;
    r /= cube(qMax(diff.size() / 2.0f, natlength / 10));
