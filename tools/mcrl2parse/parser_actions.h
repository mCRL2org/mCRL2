...
{
  action_rename_actions(const dparser::parser_table& table_)
    : lps::action_actions(table_)
  {}

  lps::action_rename_rule_rhs parse_ActionRenameRuleRHS(const dparser::parse_node& node)
  {
  ...
  lps::action_rename_rule parse_ActionRenameRule(const dparser::parse_node& node)
  {
  ...
  atermpp::vector<lps::action_rename_rule> parse_ActionRenameRuleList(const dparser::parse_node& node)
  {
  ...
  atermpp::vector<lps::action_rename_rule> parse_ActionRenameRuleSpec(const dparser::parse_node& node)
  {
  ...
  bool callback_ActionRenameSpec(const dparser::parse_node& node, lps::action_rename_specification& result)
  {
    if (symbol_name(node) == "SortSpec")
    {
      return callback_DataSpecElement(node, result.data());
  ...
  lps::action_rename_specification parse_ActionRenameSpec(const dparser::parse_node& node)
  {
