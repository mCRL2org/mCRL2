...
/*		l = split_sync(t);
	...
	if ( gsIsMerge(t) )
	{
//PrintPart_C(stderr,t);fprintf(stderr,"\n\n");
...
for (; !ATisEmpty(m); m=ATgetNext(m))
{
PrintPart_C(stderr,ATelementAt(ATLgetFirst(m),3));
fprintf(stderr," -> ");
PrintPart_C(stderr,ATelementAt(ATLgetFirst(m),1));
fprintf(stderr," . ");
PrintPart_C(stderr,ATelementAt(ATLgetFirst(m),2));
fprintf(stderr,"\n\n");
}*/
}
...
static ATermAppl make_lps(ATermAppl Spec, unsigned int init_id)
{
...
//	ATermList maps = ATLgetArgument(ATAgetArgument(Spec,2),0);
	...
	for (l=processes; !ATisEmpty(l); l=ATgetNext(l))
	{
		for (m=ATLelementAt(ATLgetFirst(l),3); !ATisEmpty(m); m=ATgetNext(m))
		{
			if ( gsIsNil(ATAelementAt(ATLgetFirst(m),2)) )
			{
				gsErrorMsg("cannot create LPS of terminating processes\n");
				exit(1);
			} else {
				o = ATLelementAt(ATLelementAt(processes,proc_id(ATAelementAt(ATLgetFirst(m),2))),1);
				if ( singleton )
...
/*static ATermAppl cluster_lps(ATermAppl spec)
{
...
{
	ATermAppl lps = (ATermAppl) ATgetArgument(spec,2);
	ATermAppl init = (ATermAppl) ATgetArgument(spec,3);

	ATermList lps_vars = (ATermList) ATgetArgument(lps,0);
	ATermList lps_pars = (ATermList) ATgetArgument(lps,1);
	ATermList init_vars = (ATermList) ATgetArgument(init,0);

	ATermTable used_names = ATtableCreate(1000,75);
	ATermList lps_pars_substs = ATmakeList0();
	ATermList lps_vars_substs = ATmakeList0();
	...
	lps_pars = make_unique(lps_pars,used_names,&lps_pars_substs);
	lps_vars = make_unique(lps_vars,used_names,&lps_vars_substs);
	init_vars = make_unique(init_vars,used_names,&init_vars_substs);

//gsprintf("lps_pars_substs: %T\n\n",lps_pars_substs);
//gsprintf("lps_vars_substs: %T\n\n",lps_vars_substs);
//gsprintf("init_vars_substs: %T\n\n",init_vars_substs);
	l = ATmakeList0();
	m = (ATermList) ATgetArgument(lps,2);
	n = ATconcat(lps_pars_substs,lps_vars_substs);
	for (; !ATisEmpty(m); m=ATgetNext(m))
	{
...
ATermAppl linearise_alt_nolps(ATermAppl Spec)
{
//	ATermList lin;
...
ATermAppl linearise_alt_nolps_subst(ATermAppl Spec, int reuse_cycles)
{
	unsigned int init_id;
...
ATermAppl linearise_alt_statespace(ATermAppl Spec, bool lps)
{
	unsigned int init_id;
