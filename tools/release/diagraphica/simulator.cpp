...
void Simulator::markFrameClusts()
{
  if (m_currentSelection == ID_FRAME_PREV)
  {
    if (0 <= m_currentSelectionIndex && static_cast <std::size_t>(m_currentSelectionIndex) < m_previousFrames.size())
    {
      emit hoverCluster(m_previousFrames[m_currentSelectionIndex], QList(m_attributes.begin(), m_attributes.end()));
    }
  }
  else if (m_currentSelection == ID_FRAME_CURR)
  {
    if (m_currentFrame != 0)
    {
      emit hoverCluster(m_currentFrame, QList(m_attributes.begin(), m_attributes.end()));
    }
  }
  else if (m_currentSelection == ID_FRAME_NEXT)
  {
    ...
    if (ids[1] == ID_FRAME_CURR || ids[1] == ID_FRAME_PREV || ids[1] == ID_FRAME_NEXT)
    {
          ...
          m_lastMouseEvent.button() == Qt::RightButton))
      {
        ...
        Cluster *frame = m_currentSelection == ID_FRAME_PREV ? m_previousFrames[m_currentSelectionIndex] :
                         ...
                         m_currentFrame;
        emit routingCluster(frame, QList<Cluster *>(), QList(m_attributes.begin(),m_attributes.end()));
      }
    }
