...
class SpringLayout
{
  ...
  float m_speed = 0.001f; ///< The rate of change each step.
  ...
  float m_attraction = 0.13f; ///< The weight of the attraction of the edges.
  const float m_min_repulsion = 0.0f;
  const float m_max_repulsion = 1.0f;
  float m_repulsion = 50.0f; ///< The weight of the repulsion of other nodes.
  const float m_min_natLength = 0.0f;
  const float m_max_natLength = 100.0f;
  float m_natLength = 50.0f; ///< The natural length of springs.
  const float m_min_handleDeviation = 0.0f;
  const float m_max_handleDeviation = 30.0f;
  float m_handleDeviation = 10.0f; ///< The natural distance of a transition handle from its straight position between the states it connect. */
  ...
  SpringLayoutUi* m_ui = nullptr; ///< The user interface generated by Qt.

  std::map<AttractionFunctionID, AttractionFunction*> attrFuncMap;
  AttractionFunction* m_attrFunc;
  AttractionFunctionID m_option_attractionCalculation = AttractionFunctionID::ltsgraph_attr;

  std::map<RepulsionFunctionID, RepulsionFunction*> repFuncMap;
  RepulsionFunction* m_repFunc;
  RepulsionFunctionID m_option_repulsionCalculation = RepulsionFunctionID::ltsgraph_rep;
...
class SpringLayoutUi : public QDockWidget
{
  ...
  SpringLayout& m_layout; ///< The layout algorithm that corresponds to this
                          ///< user interface.
  WorkerThread* m_thread = nullptr; ///< The thread that is used to calculate the new positions.
