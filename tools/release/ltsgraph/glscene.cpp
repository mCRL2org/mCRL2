...
struct VertexData
{
  ...
  void generate(const TextureData& textures, float pixelsize, float size_node)
  {
    ...
    float slice = 0, sliced = static_cast<float>(2.0 * M_PI / (RES_NODE_SLICE - 1)),
          stack = 0, stackd = static_cast<float>(M_PI_2 / RES_NODE_STACK);
    ...
    for (int j = 0; j < RES_NODE_STACK; ++j, stack += stackd)
    {
      for (int i = 0; i < RES_NODE_SLICE - 1; ++i, slice += sliced)
      {
        node[n++] = Coord3D(std::sin((stack + stackd)) * std::sin(slice),
      ...
      node[n++] = Coord3D(std::sin((stack + stackd)) * std::sin(0.0f),
    ...
    float diff = static_cast<float>(M_PI / 20.0), t = 0;
  ...
  void animate()
  {
        ...
        (m_target.world != world))
    {
      ...
      if (m_animation < halfway) {
        interpolate_cam(static_cast<float>(++m_animation) / halfway);
      }
      if (m_animation == halfway)
      {
    ...
    {
      interpolate_world(static_cast<float>(++m_animation) / m_animation_steps);
    }
    else {
      interpolate_cam(static_cast<float>(++m_animation) / m_animation_steps);
    }
  }
...
GLScene::Selection GLScene::select(int x, int y)
{
  ...
  auto bestZ = static_cast<GLuint>(-1);
  result.selectionType = so_none;
  result.index = 0;

  for (size_t index = 0; hits > 0; --hits)
  {
    GLHitRecord& rec = *(reinterpret_cast<GLHitRecord*>(&selectBuf[index]));
    index += rec.stackSize + 3;
    if ((rec.stack[0] == static_cast<GLuint>(result.selectionType) && rec.minDepth < bestZ) ||
        (rec.stack[0] > static_cast<GLuint>(result.selectionType)))
    {
