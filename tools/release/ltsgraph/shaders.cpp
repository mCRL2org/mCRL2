...
namespace{
  ...
  "void main(void)\n"
  "{\n"
  "   vec3 pos = scale * vertex;\n"
  "   float angle = acos(dot(normalize(eye - offset), normalize(eye)));\n"
  "   vec3 axis = cross(eye - offset, eye); \n"
  "   vec4 new_pos = vec4(offset, 0) + mix(vec4(pos, 1), rotationMatrix(axis, angle) * vec4(pos, 1), length(axis) > 0.01f); \n"
