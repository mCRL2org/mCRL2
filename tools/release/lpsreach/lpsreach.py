...
def run(args):
    ...
    cmd = 'lpsreach --info --groups={} {}'.format(args.groups, args.lpsfile)
    text = run_command(cmd)
    columns = parse_read_write_matrix(text)
    permutation = maximize_cost(columns, args.iterations)

    # check the permutation
    cmd = 'lpsreach --info --groups={} --reorder="{}" {}'.format(args.groups, ' '.join(list(map(str, permutation))), args.lpsfile)
    text = run_command(cmd)
    columns = parse_read_write_matrix(text)
    cost = compute_cost(columns)
    ...
    cmd = 'lpsreach -v --groups={} --reorder="{}" {}'.format(args.groups, ' '.join(list(map(str, permutation))), args.lpsfile)
    print(cmd)
    os.system(cmd)
