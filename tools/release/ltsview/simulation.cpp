...
bool Simulation::loadTrace(QString filename)
{
  ...
  {
    mCRL2log(mcrl2::log::log_level_t::error) << "The trace in " << filename.toStdString() << " has no state information, which is currently not allowed by ltsview.\nThis typically occurs with traces generated by ltscompare, which can currently not be used in ltsview.";
    return false;
  }

  if (!m_ltsRef.hasStateInfo())
  {
     mCRL2log(mcrl2::log::log_level_t::error) << "The trace in " << filename.toStdString() << " has state information but the labelled transition system does not have it, which is currently not allowed by ltsview.";
     return false;
  }
  ...
  {
    mCRL2log(mcrl2::log::log_level_t::error) << "The trace in " << filename.toStdString() << " and the labelled transition system have state information of unequal lengths.";
    return false;
  }
  ...
  while (trace.get_position() != trace.number_of_actions())
  {
    ...
    else
    {
      ...
      mCRL2log(mcrl2::log::log_level_t::error) << "Could not regenerate trace, does it belong to the loaded LTS?";
      return false;
    }
