...
QQuaternion ArcballCamera::DEFAULT_ROTATION = QQuaternion::fromDirection({1, 0, 0}, {0, 0, 1});
...
void ArcballCamera::applyDragRotate(const QPoint& p1, const QPoint& p2, const float rotation_speed){
  m_rotation = mcrl2::gui::arcballRotation(p1, p2, rotation_speed) * m_rotation;
  update();
}

void ArcballCamera::applyZoom(const QPoint& p1, const QPoint& p2, const float zoom_speed, const float zoom_base){
  QPoint diff = p2 - p1;
  m_zoom *= std::pow(zoom_base, diff.y()*zoom_speed);
