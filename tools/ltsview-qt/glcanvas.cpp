...
void GLCanvas::initialize()
{
  ...
  setBackground(settings->backgroundColor.value());
  glClearDepth(1.0);
...
void GLCanvas::display(bool coll_caller, bool selecting)
{
  ...
  if (displayAllowed)
  {
    ...
    if (!lightRenderMode || settings->navLighting.value())
    {
      glEnable(GL_NORMALIZE);
      glEnable(GL_LIGHTING);
      glEnable(GL_LIGHT0);
    ...
    if (!lightRenderMode || settings->navSmoothShading.value())
    {
    ...
    if (settings->displayWireframe.value())
    {
    ...
    if (!lightRenderMode || settings->navShowStates.value())
    {
      ...
      if (settings->displayStates.value())
      {
    ...
    if (!lightRenderMode || settings->navTransparency.value())
    {
...
unsigned char* GLCanvas::getPictureData(int w_res,int h_res)
{
  ...
  unsigned char* pixdata = (unsigned char*)malloc(w_res * h_res * 4);
  if (pixdata == NULL)
  {
    trDelete(tr_context);
    return NULL;
  ...
  trImageBuffer(tr_context, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV, pixdata);
...
void GLCanvas::setBackground(QColor value)
{
