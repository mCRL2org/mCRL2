...
architecture_diagram* grape_specification::add_architecture_diagram( uint p_id, const wxString &p_name, int p_index )
{
  int index = p_index;
...
void grape_specification::remove_architecture_diagram( architecture_diagram* p_arch_dia )
{
  ...
  if ( n != wxNOT_FOUND )
  {
    ...
    for ( uint i = 0; i < p_arch_dia->count_comment(); ++i )
    {
    ...
    for ( uint i = 0; i < p_arch_dia->count_blocked(); ++i )
    {
    ...
    for ( uint i = 0; i < p_arch_dia->count_visible(); ++i )
    {
    ...
    for ( uint i = 0; i < p_arch_dia->count_channel_communication(); ++i )
    {
    ...
    for ( uint i = 0; i < p_arch_dia->count_channel(); ++i )
    {
    ...
    for ( uint i = 0; i < p_arch_dia->count_architecture_reference(); ++i )
    {
    ...
    for ( uint i = 0; i < p_arch_dia->count_process_reference(); ++i )
    {
...
uint grape_specification::count_architecture_diagram( void )
{
...
process_diagram* grape_specification::add_process_diagram( uint p_id, const wxString &p_name, int p_index )
{
  int index = p_index;
  while ( exists_diagram( p_name, index ) )
  {
...
void grape_specification::remove_process_diagram( process_diagram* p_proc_dia )
{
  ...
  if ( n != wxNOT_FOUND )
  {
    ...
    for ( uint i = 0; i < p_proc_dia->count_comment(); ++i )
    {
    ...
    for ( uint i = 0; i < p_proc_dia->count_initial_designator(); ++i )
    {
    ...
    for ( uint i = 0; i < p_proc_dia->count_reference_state(); ++i )
    {
    ...
    for ( uint i = 0; i < p_proc_dia->count_state(); ++i )
    {
    ...
    for ( uint i = 0; i < p_proc_dia->count_terminating_transition(); ++i )
    {
    ...
    for ( uint i = 0; i < p_proc_dia->count_nonterminating_transition(); ++i )
    {
...
uint grape_specification::count_process_diagram( void )
{
...
object* grape_specification::find_object( grape_specification *p_spec, uint p_id, object_type p_type )
{
  object *result = 0;
...
void grape_specification::check_references( wxString p_name, architecture_diagram* p_arch_dia )
{
  ...
  for ( uint i = 0; i < count_architecture_diagram(); ++i )
  {
    architecture_diagram* arch_dia_ptr = get_architecture_diagram( i );
    for ( uint j = 0; j < arch_dia_ptr->count_architecture_reference(); ++j )
    {
...
void grape_specification::check_references( wxString p_name, process_diagram* p_proc_dia )
{
  ...
  {
    architecture_diagram* arch_dia_ptr = get_architecture_diagram( i );
    for ( uint j = 0; j < arch_dia_ptr->count_process_reference(); ++j )
    {
  ...
  for ( uint i = 0; i < count_process_diagram(); ++i )
  {
    process_diagram* proc_dia_ptr = get_process_diagram( i );
    for ( uint j = 0; j < proc_dia_ptr->count_reference_state(); ++j )
    {
