...
{
  fprintf(f,
    "The following output formats are accepted by " NAME ":\n"
    "\n"
    "  format  ext.  description                       remarks\n"
    "  -----------------------------------------------------------\n"
    "  aut     .aut  Aldebaran format (CADP)\n"
    "  dot     .dot  GraphViz format\n"
    "  plain   .txt  plain text format                 default\n"
    "  mcrl2   .trc  mCRL2 trace format\n"
    "\n"
    );
}
...
t_tool_options parse_command_line(int ac, char** av) {
  ...
  if (parser.options.count("out")) {
    std::string eq_name(parser.option_argument("out"));
    if (eq_name == "plain") {
      tool_options.format_for_output = otPlain;
