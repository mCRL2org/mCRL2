...
void grape::grapeapp::draw_nonterminating_transition(const coordinate p_begin, const coordinate p_control, const coordinate p_end, bool p_selected, const wxString& p_label_text)
{
  ...
  for (unsigned int i=1; i<=40; ++i)
  {
    ...
    else
    {
      ...
      if (i == 21)
      {
        angle_middle = atan2((pre_pnt.m_y-pnt.m_y), (pre_pnt.m_x-pnt.m_x)) + M_PI*0.5f;
      }
    };
  ...
  while (angle_middle < 0.0f)
  {
    angle_middle += static_cast<float>(M_PI);
  }
  while (angle_middle >= M_PI)
  {
    angle_middle -= static_cast<float>(M_PI);
  }
  ...
  if ((angle_middle >= 0.0f*M_PI) && (angle_middle < 0.25f*M_PI))
  {
    align_horizontal = al_right;
  }
  if ((angle_middle >= 0.75f*M_PI) && (angle_middle < 1.25f*M_PI))
  {
    align_horizontal = al_left;
  }

  if ((angle_middle >= 0.25f*M_PI) && (angle_middle < 0.75f*M_PI))
  {
    align_vertical = al_top;
  }
...
void grape::grapeapp::draw_terminating_transition(const coordinate& p_begin, const coordinate& p_end, bool p_selected, const wxString& p_label_text)
{
  ...
  if ((angle >= 1.25f*M_PI) && (angle < 1.75f*M_PI))
  {
    align_vertical = al_bottom;
  }
...
void grape::grapeapp::draw_channel(const coordinate& p_center, float p_radius, bool p_selected, const channel_type p_channel_type)
{
  ...
  glTranslatef(p_center.m_x, p_center.m_y, 0.0f);
  // set the color for the fill of the ellipse
  if (p_channel_type == VISIBLE_CHANNEL)
  {
    set_color(g_color_channel_visible, p_selected);
  }
  if (p_channel_type == HIDDEN_CHANNEL)
  {
    set_color(g_color_channel_hidden, p_selected);
  }
  if (p_channel_type == BLOCKED_CHANNEL)
  {
    set_color(g_color_channel_blocked, p_selected);
  }
...
void grape::grapeapp::draw_channel(const coordinate& p_center, float p_radius, bool p_selected, const channel_communication_type p_channel_communication_type)
{
  ...
  if (p_channel_communication_type == VISIBLE_CHANNEL_COMMUNICATION)
  {
    p_channel_type = VISIBLE_CHANNEL;
  }
  if (p_channel_communication_type == HIDDEN_CHANNEL_COMMUNICATION)
  {
    p_channel_type = HIDDEN_CHANNEL;
  }
  if (p_channel_communication_type == BLOCKED_CHANNEL_COMMUNICATION)
  {
    p_channel_type = BLOCKED_CHANNEL;
  }
...
grape_direction grape::grapeapp::grab_bounding_box(const coordinate& p_center, float p_width, float p_height, const coordinate& p_coord, bool p_selected)
{
  ...
  {
    coordinate left_low = { p_center.m_x - 0.5 * p_width - 2 * g_cursor_margin, p_center.m_y - 0.5 * p_height - 2 * g_cursor_margin };
    if (is_inside_rectangle(left_low, 2 * g_cursor_margin, 2 * g_cursor_margin, p_coord, false))
    {
      result = GRAPE_DIR_SOUTHWEST;
    }

    coordinate left_center = { left_low.m_x, p_center.m_y };
    if (is_inside_rectangle(left_center, 2 * g_cursor_margin, 2 * g_cursor_margin, p_coord, false))
    {
      result = GRAPE_DIR_WEST;
    }

    coordinate left_up = { left_low.m_x, p_center.m_y + 0.5 * p_height + 2 * g_cursor_margin };
    if (is_inside_rectangle(left_up, 2 * g_cursor_margin, 2 * g_cursor_margin, p_coord, false))
    {
      result = GRAPE_DIR_NORTHWEST;
    }

    coordinate center_up = { p_center.m_x, left_up.m_y };
    if (is_inside_rectangle(center_up, 2 * g_cursor_margin, 2 * g_cursor_margin, p_coord, false))
    {
      result = GRAPE_DIR_NORTH;
    }

    coordinate right_low = { p_center.m_x + 0.5 * p_width + 2 * g_cursor_margin, left_low.m_y };
    if (is_inside_rectangle(right_low, 2 * g_cursor_margin, 2 * g_cursor_margin, p_coord, false))
    {
      result = GRAPE_DIR_SOUTHEAST;
    }

    coordinate right_center = { right_low.m_x, p_center.m_y };
    if (is_inside_rectangle(right_center, 2 * g_cursor_margin, 2 * g_cursor_margin, p_coord, false))
    {
      result = GRAPE_DIR_EAST;
    }

    coordinate right_up = { right_low.m_x, left_up.m_y };
    if (is_inside_rectangle(right_up, 2 * g_cursor_margin, 2 * g_cursor_margin, p_coord, false))
    {
      result = GRAPE_DIR_NORTHEAST;
    }

    coordinate center_low = { p_center.m_x, p_center.m_y - 0.5 * p_height - 2 * g_cursor_margin };
    if (is_inside_rectangle(center_low, 2 * g_cursor_margin, 2 * g_cursor_margin, p_coord, false))
    {
      result = GRAPE_DIR_SOUTH;
    }
  }
  return result;
}
