...
class pbesbddsolve_tool : public input_output_tool
{
  protected:
    ...
    bdd_granularity parse_granularity(const std::string& value) const
    {
      if (value == "pbes")
      {
        return bdd_granularity::per_pbes;
      }
      else if (value == "equation")
      {
        return bdd_granularity::per_equation;
      }
      else if (value == "summand")
      {
        return bdd_granularity::per_summand;
      }
  ...
  public:
    ...
    bool run() override
    {
      ...
      unify_parameters(p);
      bool result = pbes_system::pbesbddsolve(p, unary_encoding, granularity, &timer()).run(apply_sylvan_optimization, remove_unreachable_vertices);
