...
int main(int argc, char** argv)
{
	...
	try {
		...
		if ( vm.count("help") > 0 )
		{
			...
			cerr << "Remove unused parts from the data specification of the LPS in INFILE, and write" << endl;
		...
		ATerm Spec;
		gsVerboseMsg("reading input LPS...\n");
		if ( filenames.size() == 0 )
		{
			Spec = readSpec(stdin);
		...
		{
			cerr << "input is not a valid LPS" << endl;
			return 1;
		}
		...
		if ( verbose )
		{
			...
			gsVerboseMsg("input LPS information\n");
		...
		{
			specification s(Spec);
			gsVerboseMsg("output LPS information\n");
			gsVerboseMsg("number of sorts: %i\n",s.data().sorts().size());
			gsVerboseMsg("number of constructors: %i\n",s.data().constructors().size());
			gsVerboseMsg("number of mappings: %i\n",s.data().mappings().size());
			gsVerboseMsg("number of equations: %i\n",s.data().equations().size());
		...
		gsVerboseMsg("writing output LPS...\n");
		if( filenames.size() == 2 )
		{
