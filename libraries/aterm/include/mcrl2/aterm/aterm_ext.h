...
{
  return ATinsert(l, static_cast_ATerm(t));
}
...
{
  return gsMakeSubst(static_cast_ATerm(old_value), static_cast_ATerm(new_value));
}
...
{
  return (ATermAppl) gsSubstValues(substs, static_cast_ATerm(appl), recursive);
}
...
{
  return (ATermList) gsSubstValues(substs, static_cast_ATerm(list), recursive);
}
