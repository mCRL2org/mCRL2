...
struct pres_actions: public data::detail::data_specification_actions
{
  ...
  pres_system::pres_expression parse_PresExpr(const core::parse_node& node) const
  {
    ...
    else if ((node.child_count() == 4) && (symbol_name(node.child(0)) == "minall") && (symbol_name(node.child(1)) == "VarsDeclList") && (symbol_name(node.child(2)) == ".") && (symbol_name(node.child(3)) == "PresExpr")) { return pres_system::minall(parse_VarsDeclList(node.child(1)), parse_PresExpr(node.child(3))); }
    else if ((node.child_count() == 4) && (symbol_name(node.child(0)) == "maxall") && (symbol_name(node.child(1)) == "VarsDeclList") && (symbol_name(node.child(2)) == ".") && (symbol_name(node.child(3)) == "PresExpr")) { return pres_system::maxall(parse_VarsDeclList(node.child(1)), parse_PresExpr(node.child(3))); }
