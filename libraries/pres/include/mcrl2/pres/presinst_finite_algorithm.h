// Author(s): Jan Friso Groote. Based on libraries/pbes/include/mcrl2/pbes/pbesinst_finite_algorithm.h by Wieger Wesselink; Alexander van Dam
...
//
/// \file mcrl2/pres/presinst_finite_algorithm.h
/// \brief add your file description here.

#ifndef MCRL2_PRES_PRESINST_FINITE_ALGORITHM_H
#define MCRL2_PRES_PRESINST_FINITE_ALGORITHM_H

#include "mcrl2/data/consistency.h"
#include "mcrl2/data/enumerator.h"
#include "mcrl2/data/replace.h"
#include "mcrl2/pres/algorithms.h"
#include "mcrl2/pres/detail/pres_parameter_map.h"
...
///        should be expanded by the finite presinst algorithm.
...
/// \brief Visitor that applies a propositional variable substitution to a pres expression.
...
struct presinst_finite_builder: public pres_system::detail::data_rewriter_builder<presinst_finite_builder<DataRewriter, SubstitutionFunction>, DataRewriter, SubstitutionFunction>
{
  ...
  const presinst_finite_rename& m_rename;
  const data::data_specification& m_data_spec;
  const presinst_index_map& m_index_map;
  const presinst_variable_map& m_variable_map;

  presinst_finite_builder(const DataRewriter& R,
                          SubstitutionFunction& sigma,
                          const presinst_finite_rename& rho,
                          const data::data_specification& data_spec,
                          const presinst_index_map& index_map,
                          const presinst_variable_map& variable_map
                         )
    : super(R, sigma),
      m_rename(rho),
  ...
  void apply(T& result, const propositional_variable_instantiation& x)
  {
    ...
    std::set<pres_expression> result_set;
    ...
    E.enumerate(enumerator_element(di_list, data::true_()),
                ...
                [&](const enumerator_element& p) {
                    ...
                    data::data_expression_list di_copy = data::data_expression_list(di_list);
                    ...
                    result_set.insert(and_(atermpp::down_cast<pres_expression>(c), propositional_variable_instantiation(Y, e_copy)));
                    return false;
                }
    );
...
/// \brief Algorithm class for the finite presinst algorithm.
class presinst_finite_algorithm
{
  protected:
    ...
    void compute_index_map(const std::vector<pres_equation>& equations,
                           const presinst_variable_map& variable_map,
                           presinst_index_map& index_map)
    {
  ...
  public:
    ...
    /// \param presspec A PRES
    /// \param variable_map A map containing the finite parameters that should be expanded by the algorithm.
    void run(pres& presspec, const presinst_variable_map& variable_map)
    {
      pres_system::algorithms::instantiate_global_variables(presspec);
      presinst_finite_rename rename;
      m_equation_count = 0;

      // compute index map corresponding to the variable map
      presinst_index_map index_map;
      compute_index_map(presspec.equations(), variable_map, index_map);

      data::rewriter rewr(presspec.data(), m_rewriter_strategy);
      ...
      for (const pres_equation& eqn: presspec.equations())
      {
        ...
        E.enumerate(enumerator_element(finite_parameter_list, data::true_()),
                    ...
                    [&](const enumerator_element& p) {
                      ...
                      detail::presinst_finite_builder<data::rewriter, data::mutable_indexed_substitution<>> visitor(rewr, sigma_j, rename, presspec.data(), index_map, variable_map);
      ...
      detail::presinst_finite_builder<data::rewriter, data::no_substitution> visitor(rewr, sigma, rename, presspec.data(), index_map, variable_map);
      propositional_variable_instantiation initial_state = visitor.visit_initial_state(presspec.initial_state());

      // assign the result
      presspec.equations() = equations;
    ...
    void run(pres& p)
    {
      ...
      presinst_variable_map variable_map;
...
void presinst_finite(pres& p, data::rewrite_strategy rewrite_strategy, const std::string& finite_parameter_selection)
{
  ...
  presinst_finite_algorithm algorithm(rewrite_strategy);
  pres_system::detail::pres_parameter_map parameter_map = pres_system::detail::parse_pres_parameter_map(p, finite_parameter_selection);
