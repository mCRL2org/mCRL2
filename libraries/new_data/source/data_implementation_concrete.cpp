...
//     new_data implementation, with the exception that sort references may occur
...
static ATermAppl impl_numerical_pattern_matching_expr(ATermAppl data_expr,
  bool top_level);
//Pre: data_expr is a new_data expression that adheres to the internal syntax after
//     new_data implementation
...
//Pre: elts is a list containing 1 or more new_data expressions, all of the same
//     sort
...
//     of sort_struct
//     p_data_decls represents a pointer to new new_data declarations, induced by
...
ATermAppl implement_data_spec(ATermAppl spec)
{
  ...
  //implement system sorts and new_data expressions occurring in spec
  ...
  //perform substitutions on new_data declarations
  ...
  //add new new_data declarations to spec
...
ATermAppl impl_data_action_rename_spec_detail(ATermAppl ar_spec, ATermAppl& lps_spec)
{
  ...
  //append new_data declarations and action declarations for the ar_spec to the lps_spec
  ...
  // We need to copy the new_data declarations of the lps_spec to the ar_spec.
...
{
  assert(gsIsProcSpec(spec) || gsIsLinProcSpec(spec) || gsIsPBES(spec) || gsIsActionRenameSpec(spec) || gsIsDataSpec(spec));
  //initalise new_data declarations
  t_data_decls data_decls;
  initialize_data_decls(&data_decls);
...
ATermAppl impl_numerical_pattern_matching(ATermAppl spec)
{
  ...
  while (!ATisEmpty(data_eqns)) {
    ...
    //implement pattern matching on the left-hand side of the new_data equation
  ...
  t_data_decls *p_data_decls)
{
  ...
  if (gsIsSortArrow(part)) {
    ...
    //part is a structured sort; replace by a new sort and add new_data
    //declarations for this sort
    part = impl_sort_struct(part, p_substs, p_data_decls);
  } else if (gsIsSortExprList(part)) {
    //part is a list sort; replace by a new sort and add new_data declarations for
    //this sort
    part = impl_sort_list(part, p_substs, p_data_decls);
  } else if (gsIsSortExprSet(part)) {
    //part is a set sort; replace by a new sort and add new_data declarations for
    //this sort
    part = impl_sort_set(part, p_substs, p_data_decls);
  } else if (gsIsSortExprBag(part)) {
    //part is a bag sort; replace by a new sort and add new_data declarations for
    //this sort
    part = impl_sort_bag(part, p_substs, p_data_decls);
  } else if (gsIsSortId(part)) {
    //part is a sort identifier; add new_data declarations for this sort, if needed
    ...
    //part is a product new_data application
    ...
    if (gsIsSetBagComp(binding_operator)) {
      ...
      //add operation identifier to the new_data declarations
      p_data_decls->ops = ATinsert(p_data_decls->ops, (ATerm) op_id);
      //add new_data equation for the operation to the new_data declarations
