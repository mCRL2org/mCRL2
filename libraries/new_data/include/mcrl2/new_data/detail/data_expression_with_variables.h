...
#define MCRL2_NEW_DATA_DATA_EXPRESSION_WITH_VARIABLES_H
...
namespace core {
  ...
  template <>
  struct term_traits<new_data::data_expression_with_variables>
  {
    /// \brief The term type
    typedef new_data::data_expression_with_variables term_type;
    ...
    static inline
    term_type true_() { return new_data::sort_bool_::true_(); }

    /// \brief The value false
    /// \return The value false
    static inline
    term_type false_() { return new_data::sort_bool_::false_(); }

    /// \brief Operator not
    /// \param p A term
    /// \return Operator not applied to p
    static inline
    term_type not_(term_type p) { return term_type(new_data::sort_bool_::not_(p), p.variables()); }
    ...
    static inline
    bool is_true(term_type t) { return new_data::sort_bool_::is_true__function_symbol(t); }

    /// \brief Test for value false
    /// \param t A term
    /// \return True if the term has the value false
    static inline
    bool is_false(term_type t) { return new_data::sort_bool_::is_false__function_symbol(t); }

    /// \brief Test for operator not
    /// \param t A term
    /// \return True if the term is of type not
    static inline
    bool is_not(term_type t) { return new_data::sort_bool_::is_not__application(t); }

    /// \brief Test for operator and
    /// \param t A term
    /// \return True if the term is of type and
    static inline
    bool is_and(term_type t) { return new_data::sort_bool_::is_and__application(t); }

    /// \brief Test for operator or
    /// \param t A term
    /// \return True if the term is of type or
    static inline
    bool is_or(term_type t) { return new_data::sort_bool_::is_or__application(t); }

    /// \brief Test for implication
    /// \param t A term
    /// \return True if the term is an implication
    static inline
    bool is_imp(term_type t) { return new_data::sort_bool_::is_implies_application(t); }
    ...
    {
      new_data::data_expression e(t);
      return e.is_abstraction() && new_data::abstraction(e).is_forall();
    }
    ...
    {
      new_data::data_expression e(t);
      return e.is_abstraction() && new_data::abstraction(e).is_exists();
    }
    ...
    {
      return term_traits<new_data::data_expression>::set_difference(v, w);
    }
