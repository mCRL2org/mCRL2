...
//
/// \file mcrl2/new_data/classic_enumerator.h
...
#include "mcrl2/new_data/detail/classic_enumerator_impl.h"
...
namespace mcrl2 {
  namespace new_data {
    ...
    class evaluator : public Rewriter {

      public:

        /// \brief Type of expressions that can be evaluated
        typedef typename Rewriter::term_type expression_type;
    };

    /** \brief Evaluator for enumeration conditions using a rewriter
     *
     * Specialised template class for evaluating enumeration conditions using a
     * rewriter on objects of type ATerm.
     * \arg MutableSubstitution a model of MutableSubstitution
     * \arg Rewriter a type for which objects are rewriter
     * \arg Selector tag identying the criterion for condition success
     *
     * Standard selectors can be found in mcrl2::new_data::selectors, at present
     * consisting of select_all, select< bool > and select_not< bool >.
     **/
    template <>
    class evaluator< new_data::detail::Rewriter > {
      ...
      private:
        ...
        boost::shared_ptr< new_data::detail::Rewriter > m_rewriter_instance;

        new_data::detail::Rewriter&                     m_rewriter;

      public:

        /// \brief Constructor for creating a rewriter with a data specification 
        evaluator(data_specification const& s) :
           m_rewriter_instance(detail::createRewriter(
                 new_data::detail::data_specification_to_aterm_data_spec(s), detail::GS_REWR_JITTY)), m_rewriter(*m_rewriter_instance) {
        }

        /// \brief Constructor for using an existing rewriter object
        evaluator(new_data::detail::Rewriter& r) : m_rewriter(r) {
        }

        /// \brief Copy constructor (reuses rewriter)
        evaluator(evaluator const& other) :
          m_rewriter_instance(other.m_rewriter_instance), m_rewriter(other.m_rewriter) {
        }

        /// \deprecated only used for classic_enumerator
        new_data::detail::Rewriter& get_rewriter() const {
          return m_rewriter;
        }
        ...
        /// \return an expression equivalent to m_rewriter(e)
        expression_type operator()(expression_type const& e) const {
          return expression_type(m_rewriter.rewrite(static_cast< ATermAppl >(e)));
        }
        ...
        expression_type operator()(expression_type const& e, MutableSubstitution const& s) const {
          using sort_bool_::true_;
          using sort_bool_::false_;

          for (typename MutableSubstitution::const_iterator i(s.begin()); i != s.end(); ++i) {
            m_rewriter.setSubstitution(static_cast< ATermAppl >(i->first),
                static_cast< ATermAppl >(static_cast< expression_type >(i->second)));
          }

          ATermAppl result = m_rewriter.rewrite(static_cast< ATermAppl >(e));

          // Subtle removal as NextStateGenerator requires substitutions for other variables
          for (typename MutableSubstitution::const_iterator i(s.begin()); i != s.end(); ++i) {
            m_rewriter.clearSubstitution(static_cast< ATermAppl >(i->first));
          }
