...
class pbes_parelm_algorithm
{
  protected:
    ...
    std::set<old_data::data_variable> unbound_variables(pbes_expression t, old_data::data_variable_list bound_variables) const
    {
    ...
    int variable_index(old_data::data_variable_list v, old_data::data_variable d) const
    {
      int index = 0;
  ...
  public:
    ...
    {
      old_data::data_variable_list fvars(p.free_variables().begin(), p.free_variables().end());
      std::vector<old_data::data_variable> predicate_variables;
      ...
      for (typename Container::const_iterator i = p.equations().begin(); i != p.equations().end(); ++i)
      {
        std::set<old_data::data_variable> uvars = unbound_variables(i->formula(), fvars);
        for (std::set<old_data::data_variable>::iterator j = uvars.begin(); j != uvars.end(); ++j)
        {
      ...
      for (typename Container::const_iterator i = p.equations().begin(); i != p.equations().end(); ++i)
      {
        ...
        old_data::data_variable_list Xparams = i->variable().parameters();
        ...
        for (std::set<propositional_variable_instantiation>::iterator j = propvars.begin(); j != propvars.end(); ++j)
        {
          ...
          old_data::data_expression_list Yparams = j->parameters();
          int Yindex = 0;
          for (old_data::data_expression_list::iterator y = Yparams.begin(); y != Yparams.end(); ++y)
          {
            std::set<old_data::data_variable> vars = old_data::find_all_data_variables(*y);
      ...
      if (mcrl2::core::gsDebug)
      {
        ...
        for(std::set<int>::iterator i = v.begin(); i != v.end(); ++i)
        {
          ...
          old_data::data_variable v1 = predicate_variables[*i];
        ...
        for( ; first != last; ++first)
        {
          ...
          old_data::data_variable v1 = predicate_variables[i1];
          int i2 = boost::target(e, G);
          core::identifier_string X2 = find_predicate_variable(p, i2);
          old_data::data_variable v2 = predicate_variables[i2];
      ...
      if (mcrl2::core::gsVerbose)
      {
        ...
        for (std::map<core::identifier_string, std::vector<int> >::const_iterator i = removals.begin(); i != removals.end(); ++i)
        {
          ...
          {
            old_data::data_variable v1 = predicate_variables[*j];
            std::cout << "(" + mcrl2::core::pp(X1) + ", " + mcrl2::core::pp(v1) + ")\n";
          }
        }
      }
