...
void replace_all_variables(T& x,
  ...
  requires (!std::is_base_of_v<atermpp::aterm, T>)
{
  core::make_update_apply_builder<pbes_system::sort_expression_builder>(sigma).update(x);
}
...
{
  T result;
  core::make_update_apply_builder<pbes_system::sort_expression_builder>(sigma).apply(result, x);
  return result;
}
