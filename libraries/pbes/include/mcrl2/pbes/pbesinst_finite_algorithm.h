...
class pbesinst_finite_algorithm
{
  ...
  public:
    ...
    explicit pbesinst_finite_algorithm(data::rewriter::strategy rewriter_strategy = data::rewrite_strategy::jitty)
      ...
      for (const pbes_equation& eqn: pbesspec.equations())
      {
        ...
        E.enumerate(enumerator_element(finite_parameter_list, data::true_()),
                    ...
                    [&](const enumerator_element& p) {
                      ...
                      mCRL2log(log::log_level_t::debug) << "Added equation " << pbes_system::pp(eqn) << "\n";
                      return false;
                    }
        );
      }
