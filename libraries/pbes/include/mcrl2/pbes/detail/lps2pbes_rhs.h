...
struct lps2pbes_counter_example_parameters: public lps2pbes_parameters
{
    ...
    : lps2pbes_parameters(phi0, lps, id_generator, T)
  {
    ...
    for (const lps::action_summand& summand: lps.action_summands())
    {
      ...
      core::identifier_string pos = id_generator("Zneg_" + utilities::number2string(actvars.size()) +  "_" + multi_action_name(ai));
      core::identifier_string neg = id_generator("Zpos_" + utilities::number2string(actvars.size()) +  "_" + multi_action_name(ai));
