...
class stategraph_local_algorithm: public stategraph_algorithm
{
  ...
  protected:
    ...
    {
      mCRL2log(log::log_level_t::debug) << "--- computed marking edge index ---" << std::endl;
      auto const& equations = m_pbes.equations();
      std::ostringstream out;
    ...
    belongs_relation compute_belongs(const local_control_flow_graph& Vk, RulesPredicate rules)
    {
      ...
      for (const stategraph_equation& eqn: equations)
      {
        ...
        mCRL2log(log::log_level_t::trace) << "  initial belong set for equation " << X << " = " << print_belong_set(eq_X, belongs) << std::endl;
        ...
        for (std::size_t i = 0; i < predvars.size(); i++)
        {
          ...
          for (auto j = belongs.begin(); j != belongs.end(); )
          {
            ...
            {
              mCRL2log(log::log_level_t::trace) << " remove (X, i, m) = (" << X << ", " << i << ", " << m << ") variable=" << eq_X.parameters()[m] << " from belongs " << std::endl;
              mCRL2log(log::log_level_t::trace) << "  used = " << print_parameters(Ye.name(), Ye.used()) << " changed = " << print_parameters(Ye.name(), Ye.changed()) << std::endl;
              belongs.erase(j++);
            }
            else
            {
        ...
        mCRL2log(log::log_level_t::trace) << "  final   belong set for equation " << X << " = " << print_belong_set(eq_X, belongs) << std::endl;
        for (std::size_t i: belongs)
        {
    ...
    void compute_belongs()
    {
      ...
      mCRL2log(log::log_level_t::debug) << "=== computing belongs relation ===" << std::endl;
      for (std::size_t k = 0; k < m_local_control_flow_graphs.size(); k++)
      {
        mCRL2log(log::log_level_t::debug) << "--- compute belongs for graph " << k << " ---" << std::endl;
    ...
    void print_belongs() const
    {
      ...
      mCRL2log(log::log_level_t::debug) << out.str() << std::endl;
    }
    ...
    {
      mCRL2log(log::log_level_t::verbose) << "--- marking statistics: " << m_marking_rewrite_count << " rewrite calls, from which " << m_marking_rewrite_cached_count << " were cached" << std::endl;
    }
    ...
    {
      mCRL2log(log::log_level_t::debug) << "=== computing control flow marking ===" << std::endl;
      ...
      for (std::size_t j = 0; j < J; j++)
      {
        ...
        mCRL2log(log::log_level_t::debug) << "--- initial control flow marking for graph " << j << "\n" << Vj.print_marking();
      }
        ...
        while (!stableint)
        {
          ...
          for (std::size_t j = 0; j < J; j++)
          {
            ...
            while (!todo.empty())
            {
              ...
              mCRL2log(log::log_level_t::trace) << " extend marking rule1: v = " << v << " marking(v) = " << core::detail::print_set(v.marking()) << std::endl;
              ...
              for (const auto& e: incoming_edges)
              {
                ...
                if (changed)
                {
                  mCRL2log(log::log_level_t::trace) << "   marking(u)' = " << core::detail::print_set(u.marking()) << std::endl;
                  todo.insert(&u);
                  stableint = false;
                }
              }
        ...
        while (!stableext)
        {
          ...
          for (std::size_t j = 0; j < J; j++)
          {
            ...
            for (const auto &u : Vj.vertices)
            {
              ...
              mCRL2log(log::log_level_t::trace) << " extend marking rule2: u = " << u << " marking(u) = " << core::detail::print_set(u.marking()) << std::endl;
              ...
              for (const auto& e: outgoing_edges)
              {
                ...
                for (std::size_t i: labels)
                {
                  ...
                  for (std::size_t k = 0; k < J; k++)
                  {
                    ...
                    {
                      auto const& v = *vk;
                      mCRL2log(log::log_level_t::trace) << "     v = " << v << " marking(v) = " << core::detail::print_set(v.marking()) << std::endl;
                      if (v.name() != Y)
                      {
                        continue;
                      }
    ...
    void compute_control_flow_marking_using_edge_index()
    {
      ...
      while (!todo.empty())
      {
        ...
        mCRL2log(log::log_level_t::trace) << "    rule: considering equation " << X << std::endl;
        mCRL2log(log::log_level_t::trace) << "    rule2: considering PVI nr. " << i << std::endl;
    ...
    void print_control_flow_marking() const
    {
      ...
      for (std::size_t k = 0; k < K; k++)
      {
        ...
        mCRL2log(log::log_level_t::debug) <<  "--- computed control flow marking for graph " << k << "\n" << Gk.print_marking() << std::endl;
      }
    }
                        ...
                        std::size_t k)
    {
      for (const auto& i: Bk)
      {
        ...
        for (const data::variable& v: V)
        {
          ...
          if (j != B_X.end())
          {
            mCRL2log(log::log_level_t::debug) <<  "removing belongs variable " << *j << " in B[" << X << "] , since it already appears in belongs relation " << k << std::endl;
            B_X.erase(*j);
          }
        }
      }
    ...
    {
      mCRL2log(log::log_level_t::trace) << "--- computing extra local control flow graph" << std::endl;
      ...
      for (const auto& eq_X: equations)
      {
        ...
        mCRL2log(log::log_level_t::trace) << "initial belongs relation B[" << X << "] = " << core::detail::print_set(B_X) << std::endl;
      ...
      mCRL2log(log::log_level_t::debug) << "--- extra local control flow graph\n" << m_local_control_flow_graphs.back() << std::endl;
      mCRL2log(log::log_level_t::debug) << "--- belongs relation for extra graph\n" << print_belongs(B);
    }
    ...
    void compute_local_control_flow_graph(const std::set<local_control_flow_graph_vertex>& U, const std::map<core::identifier_string, std::size_t>& C)
    {
      ...
      mCRL2log(log::log_level_t::debug) << "--- compute_local_control_flow_graph for vertices " << core::detail::print_set(U) << std::endl;
      local_control_flow_graph V;
      std::set<const local_control_flow_graph_vertex*> todo;
      ...
      while (!todo.empty())
      {
        ...
        mCRL2log(log::log_level_t::trace) << "choose todo element (X, k, d=e) = " << u << std::endl;
        ...
        for (auto i = predvars.begin(); i != predvars.end(); ++i)
        {
          ...
          {
            // case 1: (X, ?, ?=?) -> (Y, k', d'=e')
            mCRL2log(log::log_level_t::trace) << "case 1" << std::endl;
            if (q != C.end()) // (Y, k1) in C
            {
              ...
              {
                // target(X, i, k') = e'
                mCRL2log(log::log_level_t::trace) << "case 1: (X, e) -> (Y, d', e') ; target(X, i, k') = e' ; k' = " << print_index(k1) << std::endl;
            ...
            else
            {
              if (X != Y)
              {
                mCRL2log(log::log_level_t::trace) << "case 2: (X, ?) -> (Y, ?)" << std::endl;
          ...
          else
          {
            ...
            if (q != C.end()) // (Y, k') in C
            {
              ...
              if (is_mapped_to(i->source(), k, e))
              {
                ...
                auto e1 = i->target(k1);
                mCRL2log(log::log_level_t::trace) << "case 3a: (X, d, e) -> (Y, d', e') ; source(X, i, k) = e && target(X, i, k') = e' ; k' = " << print_index(k1) << std::endl;
                if (e1 != data::undefined_data_expression())
                {
                  V.insert_edge(todo, m_pbes, u, Y, k1, e1, edge_label);
                }
              ...
              else if (Y != X && is_undefined(i->source(), k))
              {
                ...
                if (e1 != data::undefined_data_expression())
                {
                  mCRL2log(log::log_level_t::trace) << "case 3b: (X, d, e) -> (Y, d', e') ; Y != X && undefined(source(X, i, k)) && target(X, i, k') = e' ; k' = " << print_index(k1) << std::endl;
                  V.insert_edge(todo, m_pbes, u, Y, k1, e1, edge_label);
                }
                ...
                if (i->copy(k) == k1)
                {
                  mCRL2log(log::log_level_t::trace) << "case 3c: (X, d, e) -> (Y, d', e') ; Y != X && undefined(source(X, i, k)) && copy(X, i, k) = k' ; k' = " << print_index(k1) << std::endl;
            ...
            else
            {
              ...
              if (e1 == e || e1 == data::undefined_data_expression())
              {
                ...
                mCRL2log(log::log_level_t::trace) << "case 4: (X, d, e) -> (Y, ?)" << std::endl;
    ...
    {
      mCRL2log(log::log_level_t::debug) << "Compute local control flow graphs for component " << print_connected_component(component) << std::endl;
    ...
    void print_local_control_flow_graphs() const
    {
      ...
      {
        mCRL2log(log::log_level_t::debug) << "--- computed local control flow graph " << (i - m_local_control_flow_graphs.begin()) << "\n" << *i << std::endl;
      }
    }
  ...
  public:
    ...
    void execute_marking_step()
    {
      ...
      switch (m_options.marking_algorithm)
      {
        ...
        case 1: {
          compute_edge_index();
          mCRL2log(log::log_level_t::trace) << print_edge_index() << std::endl;
