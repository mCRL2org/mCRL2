...
///////////////////////////////////////////////////////////////////////////////
// timed_action
/// \brief multi lps::action with time
///
class timed_action
{
  protected:
...
struct not_equal_data_parameters_builder
{
  ...
  not_equal_data_parameters_builder(const std::vector<lps::action>& a_,
                                    const std::vector<lps::action>& b_,
                                    atermpp::vector<pbes_expression>& result_
                                   )
    : a(a_),
...
inline
data::data_variable_list Par(core::identifier_string x, data::data_variable_list l, state_formula f)
{
  ...
  if (is_data(f)) {
    ...
    {
      data::data_variable_list xf = mu_variables(f);
...
namespace pbes_timed
{
  ...
  pbes_expression RHS(state_formula f0, state_formula f, lps::linear_process lps, data::data_variable T, std::set<std::string>& context)
  {
    ...
    if (s::is_data(f)) {
      ...
      data::data_expression_list d = mu_expressions(f);
      data::data_variable_list xp = lps.process_parameters();
      result = propositional_variable_instantiation(X, T + d + xp + Par(X, data::data_variable_list(), f0));
    } else {
  ...
  /// f0 is the original formula
  inline
  atermpp::vector<pbes_equation> E(state_formula f0, state_formula f, lps::linear_process lps, data::data_variable T)
  {
    using namespace modal::state_frm;
...
namespace pbes_untimed
{
  ...
  /// f0 is the original formula
  inline
  atermpp::vector<pbes_equation> E(state_formula f0, state_formula f, lps::linear_process lps)
  {
    using namespace modal::state_frm;
    atermpp::vector<pbes_equation> result;
