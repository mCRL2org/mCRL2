...
std::cout << "RIGHT AFTER\n"; print_expression(right);
  ...
  pbes_expression make_and(const pfnf_visitor_expression& left, const pfnf_visitor_expression& right) const
  {
    return utilities::optimized_and(left.expr, right.expr);
  }

  pbes_expression make_or(const pfnf_visitor_expression& left, const pfnf_visitor_expression& right) const
  {
    return utilities::optimized_or(left.expr, right.expr);
  }

  pbes_expression make_not(const pfnf_visitor_expression& x) const
  {
  ...
  {
    expression_stack.push_back(pfnf_visitor_expression(x));
  }
...
//std::cout << "AND RESULT\n"; print_expression(pfnf_visitor_expression(h, q, g));
  ...
  void leave(const or_&)
  {
    ...
    {
      g.push_back(pfnf_visitor_implication(make_and(not_h_psi, i->g), i->rhs));
    }

    // second conjunction
    for (atermpp::vector<pfnf_visitor_implication>::const_iterator i = q_psi.begin(); i != q_psi.end(); ++i)
    {
      g.push_back(pfnf_visitor_implication(make_and(not_h_phi, i->g), i->rhs));
    }
    ...
    for (atermpp::vector<pfnf_visitor_implication>::const_iterator i = q_phi.begin(); i != q_phi.end(); ++i)
    {
      ...
      {
        g.push_back(pfnf_visitor_implication(make_and(i->g, k->g), concat(i->rhs, k->rhs)));
      }
    }
//std::cout << "OR RESULT\n"; print_expression(pfnf_visitor_expression(h, q, g));
    expression_stack.push_back(pfnf_visitor_expression(h, q, g));
  ...
  void enter(const propositional_variable_instantiation& x)
  {
    ...
    atermpp::vector<pfnf_visitor_implication> g(1, pfnf_visitor_implication(true_(), atermpp::vector<propositional_variable_instantiation>(1, x)));
    expression_stack.push_back(pfnf_visitor_expression(h, q, g));
  }
};
