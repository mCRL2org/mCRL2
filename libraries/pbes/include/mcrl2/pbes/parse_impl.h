...
struct pbes_actions: public data::detail::data_specification_actions
{
  ...
  {
    if ((node.child_count() == 1) && (symbol_name(node.child(0)) == "DataValExpr")) { return atermpp::down_cast<pbes_system::pbes_expression>(parse_DataValExpr(node.child(0))); }
    else if ((node.child_count() == 1) && (symbol_name(node.child(0)) == "DataExpr")) { return atermpp::down_cast<pbes_system::pbes_expression>(parse_DataExpr(node.child(0))); }
    ...
    else if ((node.child_count() == 2) && (symbol_name(node.child(0)) == "Id")) { return atermpp::down_cast<pbes_system::pbes_expression>(data::untyped_data_parameter(parse_Id(node.child(0)), parse_DataExprList(node.child(1)))); }
