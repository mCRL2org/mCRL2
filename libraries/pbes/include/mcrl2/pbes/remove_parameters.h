...
struct remove_parameters_builder: public pbes_system::pbes_expression_builder<Derived>
{
  ...
  {
    make_propositional_variable(result, x.name(), detail::remove_elements(x.parameters(), to_be_removed));
  }
...
/// \param x A PBES library object that derives from atermpp::aterm
...
struct set_based_remove_parameters_builder: public pbes_expression_builder<Derived>
{
  ...
  void apply(T& result, const propositional_variable& x)
  {
    ...
    make_propositional_variable(result, x.name(), vars);
  }
