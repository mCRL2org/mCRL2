...
%TCIDATA{LastRevised=Tuesday, January 15, 2008 15:07:02}
...
be a multi-action, or have one of the special values $\delta $ or $\tau $.
We define four pbes equation systems that express some kind of bisimulation
equivalence between $M$ and $S$.
...
close_{1,i}^{p,q}(d,d^{\prime },e) &=&\bigvee\limits_{\{j\in
I_{q}|a_{j}=\tau \}}\exists _{e^{\prime }:E_{j}}.(c_{j}(d^{\prime
},e^{\prime })\wedge Y_{1,i}^{p,q}(d,g_{j}(d^{\prime },e^{\prime }),e))\vee
step_{i}^{p,q}(d,d^{\prime },e) \\
&& \\
step_{i}^{p,q}(d,d^{\prime },e) &=&\left\{
\begin{array}{cl}
