...
%TCIDATA{LastRevised=Tuesday, December 06, 2011 16:46:54}
...
The Absinthe algorithm takes as input a substitution on function symbols $%
\sigma _{F}:\mathcal{F}\rightarrow \mathcal{F}$, and a substitution on sorts 
$\sigma _{S}:\mathcal{S}\rightarrow \mathcal{S}$, where $\mathcal{F}$ is the
set of function symbols and $\mathcal{S}$ is the set of sorts. For each $%
s\in domain(\sigma _{S}\mathcal{)}$ there is a corresponding abstraction
function $h_{s}$.

\subsection{Extending $\protect\sigma _{F}$}

The algorithm extends the substitution $\sigma _{F}$ for all function
symbols $f$ that are used in the PBES, but are not present yet in $\sigma
_{F}$. A function symbol $f\in \mathcal{F}$ is "generalized" as follows:%
\begin{equation*}
\begin{array}{lcl}
f:s_{b} & \mapsto  & \overline{f}:s_{b}^{\sigma _{S}} \\ 
f:C(s) & \mapsto  & \overline{f}:C(s)^{\sigma _{S}} \\ 
f:s_{1}\times \cdots \times s_{n}\rightarrow s & \mapsto  & \overline{f}%
:s_{1}^{\sigma _{S}}\times \cdots s_{n}^{\sigma _{S}}\rightarrow
Set(s^{\sigma _{S}}),%
\end{array}%
\end{equation*}%
where $s_{b}$ is a basic sort, $s_{1}\times \cdots \times s_{n}\rightarrow s$
is a function sort, and $C(s)$ is a container sort. The sort of $f$ may not
contain any element of $domain(\sigma _{S}\mathcal{)}$ as a subsort (N.B.
this is probably too strict!).

For each of the function symbols a corresponding equation is introduced:%
\begin{equation*}
\begin{array}{lcl}
\overline{f} & = & \left\{ 
\begin{array}{ll}
h_{s_{b}}(f)\text{ } & \text{if }s_{b}\in domain(\sigma _{S}\mathcal{)} \\ 
f^{\sigma _{S}} & \text{otherwise}%
\end{array}%
\right.  \\ 
&  &  \\ 
\overline{f} & = & f^{\sigma _{S}} \\ 
&  &  \\ 
\overline{f}(x) & = & \left\{ 
\begin{array}{ll}
\{h_{s}(f(x))\}\text{ } & \text{if }s\in domain(\sigma _{S}\mathcal{)} \\ 
\{f(x)\} & \text{otherwise,}%
\end{array}%
\right. 
\end{array}%
\end{equation*}%
where $x:s_{1}\times \cdots \times s_{n}\rightarrow s$. Besides this, the
substitution $\sigma _{F}$ is extended with constructor functions for
container sorts of the sorts in $domain(\sigma _{S}\mathcal{)}$. 
