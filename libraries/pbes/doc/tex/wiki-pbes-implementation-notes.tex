...
%TCIDATA{LastRevised=Friday, December 09, 2011 17:08:00}
...
The algorithm first extends the substitution $\sigma _{F}$. Let $A=\left(
FunctionSymbols(p)\cup \dbigcup\limits_{s\in domain(\sigma
_{S})}ContainerConstructors(s)\right) \setminus domain(\sigma _{F}\mathcal{)}
$. For each function symbol $f\in A$ the substitution $\sigma _{F}$ is
extended based on the sort of $f$:%
\begin{equation*}
\begin{array}{lcl}
\sigma _{F}(f:s_{b}) & = & \overline{f}:s_{b}^{\sigma _{S}} \\ 
\sigma _{F}(f:C(s)) & = & \overline{f}:C(s)^{\sigma _{S}} \\ 
\sigma _{F}(f:s_{1}\times \cdots \times s_{n}\rightarrow s) & = & \overline{f%
}:s_{1}^{\sigma _{S}}\times \cdots s_{n}^{\sigma _{S}}\rightarrow
Set(s^{\sigma _{S}}),%
\end{array}%
\end{equation*}%
where $s_{b}$ is a basic sort, $s_{1}\times \cdots \times s_{n}\rightarrow s$
is a function sort, and $C(s)$ is a container sort, and where $\overline{f}$
is a fresh identifier. The sort of $f$ may not contain any element of $%
domain(\sigma _{S}\mathcal{)}$ as a subsort.
...
\end{array}%
\end{equation*}%
where $x:s_{1}\times \cdots \times s_{n}\rightarrow s$. 
