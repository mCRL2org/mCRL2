...
%TCIDATA{LastRevised=Tuesday, October 26, 2010 17:14:35}
...
_{j})_{m},0,\cdots ,0] & \text{\textsf{if}}\ \mathsf{rank}(X_{j})\text{ is
even} \\ 
\mathsf{inc}([(\mathcal{\alpha }_{j})_{1},\cdots ,(\mathcal{\alpha }%
_{j})_{m},0,\cdots ,0],m) & \text{\textsf{if}}\ \mathsf{rank}(X_{j})\text{
is odd}%
\end{array}%
\right. 
\end{equation*}%
