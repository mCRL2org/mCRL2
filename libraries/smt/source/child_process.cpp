...
std::string child_process::read() const
{
  ...
  do
  {
    bSuccess = ReadFile( m_pipes->g_hChildStd_OUT_Rd, output + totalRead, 512 - totalRead, &dwRead, NULL);
    totalRead += dwRead;
...
void child_process::initialize()
{
  ...
  if (!CreatePipe(&m_pipes->g_hChildStd_OUT_Rd, &m_pipes->g_hChildStd_OUT_Wr, &saAttr, 0))
  {
    throw mcrl2::runtime_error("Could not connect to SMT solver: failed to create pipe Stdout");
  }
  // Ensure the read handle to the pipe for STDOUT is not inherited.
  if(!SetHandleInformation(m_pipes->g_hChildStd_OUT_Rd, HANDLE_FLAG_INHERIT, 0))
  {
    throw mcrl2::runtime_error("Could not modify SMT solver handle: SetHandleInformation Stdout");
  }
  // Create a pipe for the child process's STDIN.
  if(!CreatePipe(&m_pipes->g_hChildStd_IN_Rd, &m_pipes->g_hChildStd_IN_Wr, &saAttr, 0))
  {
    throw mcrl2::runtime_error("Could not connect to SMT solver: failed to create pipe Stdin");
  }
  // Ensure the write handle to the pipe for STDIN is not inherited.
  if(!SetHandleInformation(m_pipes->g_hChildStd_IN_Wr, HANDLE_FLAG_INHERIT, 0))
  {
  ...
  siStartInfo.hStdError = m_pipes->g_hChildStd_OUT_Wr;
  siStartInfo.hStdOutput = m_pipes->g_hChildStd_OUT_Wr;
  siStartInfo.hStdInput = m_pipes->g_hChildStd_IN_Rd;
...
child_process::~child_process()
{
  ...
  CloseHandle(m_pipes->g_hChildStd_IN_Wr);
}
...
void child_process::write(const std::string& command) const
{
  if(::write(m_pipes->pipe_stdin[1], command.c_str(), command.size()) < 0)
  {
...
std::string child_process::read() const
{
  char output[512];
  // check return value
  int count = ::read(m_pipes->pipe_stdout[0], output, 512);
  if(count > 0)
  {
  ...
  else
  {
    ...
    while(int i = ::read(m_pipes->pipe_stderr[0], output, 512))
    {
...
void child_process::initialize()
{
  ...
  if (::pipe(&m_pipes->pipe_stdin[0]) < 0)
  {
    throw mcrl2::runtime_error("failed to create pipe");
  }

  if (::pipe(&m_pipes->pipe_stdout[0]) < 0)
  {
    throw mcrl2::runtime_error("failed to create pipe");
  }

  if (::pipe(&m_pipes->pipe_stderr[0]) < 0)
  {
    throw mcrl2::runtime_error("failed to create pipe");
  }
  ...
  if (pid == 0)
  {
    ...
    ::dup2(m_pipes->pipe_stdin[0], STDIN_FILENO);
    ::dup2(m_pipes->pipe_stdout[1], STDOUT_FILENO);
    ::dup2(m_pipes->pipe_stderr[1], STDERR_FILENO);

    ::close(m_pipes->pipe_stdin[1]);
    ::close(m_pipes->pipe_stdout[0]);
    ::close(m_pipes->pipe_stderr[0]);
  ...
  {
    ::close(m_pipes->pipe_stdin[0]);
    ::close(m_pipes->pipe_stdin[1]);
    ::close(m_pipes->pipe_stdout[0]);
    ::close(m_pipes->pipe_stdout[1]);
    ::close(m_pipes->pipe_stderr[0]);
    ::close(m_pipes->pipe_stderr[1]);
