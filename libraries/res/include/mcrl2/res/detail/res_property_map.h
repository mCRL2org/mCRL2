// Author(s): Jan Friso Groote. Based on bes_property_map by Wieger Wesselink, Jeroen Keiren
...
//
/// \file mcrl2/res/detail/res_property_map.h
/// \brief A property map containing properties of an LPS specification.

#ifndef MCRL2_RES_DETAIL_RES_PROPERTY_MAP_H
#define MCRL2_RES_DETAIL_RES_PROPERTY_MAP_H
...
class res_property_map : public mcrl2::data::detail::data_property_map< res_property_map >
{
  protected:

    // Allow base class access to protected functions
    friend class data::detail::data_property_map< res_property_map >;

    typedef data::detail::data_property_map< res_property_map > super;

    using super::print;

    std::string print(const res_variable& v) const
    {
      return res::pp(v);
    }
    ...
    std::pair<unsigned int, unsigned int> compute_equation_counts(const res_equation_system& b) const
    {
    ...
    unsigned int compute_block_nesting_depth(const res_equation_system& b) const
    {
  ...
  public:
    ...
    res_property_map(const std::string& text)
      : super(text)
    {}

    /// \brief Constructor
    /// Initializes the res_property_map with a res equation system
    res_property_map(const res_equation_system& b)
    {
      ...
      std::set<res_variable>             binding_variables            = b.binding_variables();
      std::set<res_variable>             occurring_variables          = b.occurring_variables();
    ...
    std::string compare(const res_property_map& other) const
    {
...
} // namespace res_equation_system

} // namespace mcrl2

#endif // MCRL2_RES_DETAIL_RES_PROPERTY_MAP_H
