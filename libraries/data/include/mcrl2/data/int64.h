...
#ifndef MCRL2_DATA_INT64_H
#define MCRL2_DATA_INT64_H
...
namespace mcrl2::data::sort_int
{
      ...
      data_equation_vector int_generate_equations_code()
      {
        ...
        result.emplace_back(variable_list({vn}), sort_nat::equals_zero(vn), negate(vn), cint(sort_nat::most_significant_digit_nat(sort_machine_word::zero_word())));
        result.emplace_back(variable_list({vn}), sort_nat::not_equals_zero(vn), negate(vn), cneg(sort_nat::nat2pos(vn)));
        ...
        result.emplace_back(variable_list({vn}), pred(vn), if_(sort_nat::equals_zero(vn), cneg(sort_pos::most_significant_digit(sort_machine_word::one_word())), cint(sort_nat::natpred(vn))));
        ...
        result.emplace_back(variable_list({vp, vq}), less_equal(vq, vp), minus(vp, vq), cint(sort_nat::monus(sort_nat::pos2nat(vp), sort_nat::pos2nat(vq))));
        result.emplace_back(variable_list({vp, vq}), less(vp, vq), minus(vp, vq), negate(sort_nat::monus(sort_nat::pos2nat(vq), sort_nat::pos2nat(vp))));
        ...
        result.emplace_back(variable_list({vn, vp}), exp(cneg(vp), vn), if_(sort_nat::is_odd(vn), cneg(exp(vp, vn)), cint(sort_nat::pos2nat(exp(vp, vn)))));
        return result;
      }

} // namespace mcrl2::data::sort_int

#endif // MCRL2_DATA_INT64_H
