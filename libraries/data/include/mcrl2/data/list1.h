...
namespace mcrl2::data::sort_list
{
      ...
      data_equation_vector list_generate_equations_code(const sort_expression& s)
      {
        ...
        result.emplace_back(variable_list({vd, vs}), element_at(s, cons_(s, vd, vs), sort_nat::c0()), vd);
        result.emplace_back(variable_list({vd, vp, vs}), element_at(s, cons_(s, vd, vs), sort_nat::cnat(vp)), element_at(s, vs, sort_nat::pred(vp)));
