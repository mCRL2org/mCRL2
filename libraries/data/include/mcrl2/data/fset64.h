...
namespace mcrl2::data::sort_fset {
  ...
  data_equation_vector fset_generate_equations_code(const sort_expression& s)
  {
    ...
    result.emplace_back(variable_list({vd, ve, vs, vt}),
        equal_to(cons_(s, vd, vs), cons_(s, ve, vt)),
        sort_bool::and_(equal_to(vd, ve), equal_to(vs, vt)));
    ...
    result.emplace_back(variable_list({vd, ve, vs, vt}),
        less_equal(cons_(s, vd, vs), cons_(s, ve, vt)),
        if_(less(vd, ve),
            sort_bool::false_(),
            if_(equal_to(vd, ve), less_equal(vs, vt), less_equal(cons_(s, vd, vs), vt))));
    ...
    result.emplace_back(variable_list({vd, ve, vs, vt}),
        less(cons_(s, vd, vs), cons_(s, ve, vt)),
        if_(less(vd, ve), sort_bool::false_(), if_(equal_to(vd, ve), less(vs, vt), less_equal(cons_(s, vd, vs), vt))));
    ...
    result.emplace_back(variable_list({vd, ve, vs}),
        less(vd, ve),
        insert(s, vd, cons_(s, ve, vs)),
        cons_(s, vd, cons_(s, ve, vs)));
    result.emplace_back(variable_list({vd, ve, vs}),
        less(ve, vd),
        insert(s, vd, cons_(s, ve, vs)),
        cons_(s, ve, insert(s, vd, vs)));
    ...
    result.emplace_back(variable_list({vd, ve, vs}),
        in(s, vd, cons_(s, ve, vs)),
        sort_bool::or_(equal_to(vd, ve), in(s, vd, vs)));
    result.emplace_back(variable_list({vd, ve, vs}),
        in(s, vd, insert(s, ve, vs)),
        sort_bool::or_(equal_to(vd, ve), in(s, vd, vs)));
    ...
    result.emplace_back(variable_list({vd, vs, vt}),
        difference(s, cons_(s, vd, vs), cons_(s, vd, vt)),
        difference(s, vs, vt));
    result.emplace_back(variable_list({vd, ve, vs, vt}),
        less(vd, ve),
        difference(s, cons_(s, vd, vs), cons_(s, ve, vt)),
        cons_(s, vd, difference(s, vs, cons_(s, ve, vt))));
    result.emplace_back(variable_list({vd, ve, vs, vt}),
        less(ve, vd),
        difference(s, cons_(s, vd, vs), cons_(s, ve, vt)),
        difference(s, cons_(s, vd, vs), vt));
    result.emplace_back(variable_list({vs}), union_(s, vs, empty(s)), vs);
    result.emplace_back(variable_list({vt}), union_(s, empty(s), vt), vt);
    result.emplace_back(variable_list({vd, vs, vt}),
        union_(s, cons_(s, vd, vs), cons_(s, vd, vt)),
        cons_(s, vd, union_(s, vs, vt)));
    result.emplace_back(variable_list({vd, ve, vs, vt}),
        less(vd, ve),
        union_(s, cons_(s, vd, vs), cons_(s, ve, vt)),
        cons_(s, vd, union_(s, vs, cons_(s, ve, vt))));
    result.emplace_back(variable_list({vd, ve, vs, vt}),
        less(ve, vd),
        union_(s, cons_(s, vd, vs), cons_(s, ve, vt)),
        cons_(s, ve, union_(s, cons_(s, vd, vs), vt)));
    ...
    result.emplace_back(variable_list({vd, vs, vt}),
        intersection(s, cons_(s, vd, vs), cons_(s, vd, vt)),
        cons_(s, vd, intersection(s, vs, vt)));
    result.emplace_back(variable_list({vd, ve, vs, vt}),
        less(vd, ve),
        intersection(s, cons_(s, vd, vs), cons_(s, ve, vt)),
        intersection(s, vs, cons_(s, ve, vt)));
    result.emplace_back(variable_list({vd, ve, vs, vt}),
        less(ve, vd),
        intersection(s, cons_(s, vd, vs), cons_(s, ve, vt)),
        intersection(s, cons_(s, vd, vs), vt));
