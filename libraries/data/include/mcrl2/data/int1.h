...
#ifndef MCRL2_DATA_INT1_H
#define MCRL2_DATA_INT1_H
...
namespace mcrl2::data::sort_int
{
      ...
      /// \param[out] result The data expression where the \@cInt expression is put.
      ...
      /// \param[out] result The data expression where the \@cNeg expression is put.
      ...
      /// \param[out] result The data expression where the Nat2Int expression is put.
      ...
      /// \param[out] result The data expression where the Int2Nat expression is put.
      ...
      /// \param[out] result The data expression where the Pos2Int expression is put.
      ...
      /// \param[out] result The data expression where the Int2Pos expression is put.
      ...
      data_equation_vector int_generate_equations_code()
      {
        ...
        result.emplace_back(variable_list({vp}), abs(cneg(vp)), sort_nat::cnat(vp));
        ...
        result.emplace_back(variable_list({vp}), negate(sort_nat::cnat(vp)), cneg(vp));
        result.emplace_back(variable_list({vn}), negate(cint(vn)), negate(vn));
        result.emplace_back(variable_list({vp}), negate(cneg(vp)), cint(sort_nat::cnat(vp)));
        result.emplace_back(variable_list({vn}), succ(cint(vn)), cint(sort_nat::cnat(succ(vn))));
        result.emplace_back(variable_list({vp}), succ(cneg(vp)), negate(pred(vp)));
        result.emplace_back(variable_list(), pred(sort_nat::c0()), cneg(sort_pos::c1()));
        result.emplace_back(variable_list({vp}), pred(sort_nat::cnat(vp)), cint(pred(vp)));
        ...
        result.emplace_back(variable_list({vn, vp}), times(cint(vn), cneg(vp)), negate(times(sort_nat::cnat(vp), vn)));
        result.emplace_back(variable_list({vn, vp}), times(cneg(vp), cint(vn)), negate(times(sort_nat::cnat(vp), vn)));
        result.emplace_back(variable_list({vp, vq}), times(cneg(vp), cneg(vq)), cint(sort_nat::cnat(times(vp, vq))));
        ...
        result.emplace_back(variable_list({vn, vp}), sort_nat::even(vn), exp(cneg(vp), vn), cint(sort_nat::cnat(exp(vp, vn))));
        result.emplace_back(variable_list({vn, vp}), sort_bool::not_(sort_nat::even(vn)), exp(cneg(vp), vn), cneg(exp(vp, vn)));
        return result;
      }

} // namespace mcrl2::data::sort_int

#endif // MCRL2_DATA_INT1_H
