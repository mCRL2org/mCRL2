...
namespace mcrl2::data::sort_list
{
      ...
      data_equation_vector list_generate_equations_code(const sort_expression& s)
      {
        ...
        result.emplace_back(variable_list({vd, vs, vt}), concat(s, cons_(s, vd, vs), vt), cons_(s, vd, concat(s, vs, vt)));
        result.emplace_back(variable_list({vs}), concat(s, vs, empty(s)), vs);
        result.emplace_back(variable_list({vd, vn, vs}), element_at(s, cons_(s, vd, vs), vn), if_(equal_to(vn, sort_nat::most_significant_digit_nat(sort_machine_word::zero_word())), vd, element_at(s, vs, sort_nat::natpred(vn))));
        ...
        result.emplace_back(variable_list({vd, ve, vs}), rhead(s, cons_(s, vd, cons_(s, ve, vs))), rhead(s, cons_(s, ve, vs)));
        result.emplace_back(variable_list({vd}), rtail(s, cons_(s, vd, empty(s))), empty(s));
        result.emplace_back(variable_list({vd, ve, vs}), rtail(s, cons_(s, vd, cons_(s, ve, vs))), cons_(s, vd, rtail(s, cons_(s, ve, vs))));
        return result;
      }
