...
//------------------------------------------//
// will be generated in ABC_parser.cpp
//------------------------------------------//
int ABC_parse(void);
extern YYSTYPE ABC_lval;
extern int ABC_debug;

//------------------------------------------//
// global variables, used in ABC_parser.cpp
//------------------------------------------//
ATerm ABC_tree = NULL;                    // the parse tree
ATermIndexedSet ABC_protect_table = NULL; // table to protect parsed ATerms

//------------------------------------------//
// ABC_FlexLexer implementation
//------------------------------------------//
int ABC_FlexLexer::yywrap(void) {
  return 1;
}

int ABC_FlexLexer::yylex(void)
{
  return 1;
}
...
// class ABC_lexer
//------------------------------------------//
// N.B. It is not possible to separate this class definition from the .ll file.
class ABC_lexer : public ABC_FlexLexer
{
  public:
    // constructor
    ABC_lexer(bool print_parse_errors) : ABC_FlexLexer(NULL, NULL)
    {
      line_nr = 1;
      col_nr = 1;
      cur_streams = NULL;
    ...
    void yyerror(const char* s)
    {
      if (show_errors)
      {
        ...
        std::cerr << "token '" << YYText() << "'"
                  << " at position " << line_nr << ", " << oldcol_nr
                  << " caused the following error: " << std::string(s) << "\n";
      }
    }
    ...
    ATerm parse(std::vector<std::istream*> &streams)
    {
      ...
      ABC_tree = NULL;
      ATprotect(&ABC_tree);
      ABC_protect_table = ATindexedSetCreate(10000, 50);
      line_nr = 1;
      col_nr = 1;
      cur_index = 0;
      cur_streams = &streams;
      ...
      {
        //ABC_tree contains the parsed specification
        result = ABC_tree;
        ABC_tree = NULL;
      }
      ATindexedSetDestroy(ABC_protect_table);
      ATunprotect(&ABC_tree);
      return result;
    }
  ...
  protected:
    ...
    // update position, provide token to parser
    void process_string(void)
    {
      col_nr += YYLeng();
      ABC_lval.appl = mcrl2::core::detail::gsString2ATermAppl(YYText());
    }
};

//implement yylex in ABC_lexer instead of ABC_FlexLexer
//(this gets rid of global variables but is ugly in its own right)
#define YY_DECL int ABC_lexer::yylex()

// global lexer object, used by ABC_parse
ABC_lexer *an_ABC_lexer = NULL;

int ABC_lex()
{
  return an_ABC_lexer->yylex();
}

void ABC_error(const char *s)
{
  return an_ABC_lexer->yyerror(s);
}

ATerm ABC_parse(std::vector<std::istream*> &streams, bool print_parse_errors)
{
  an_ABC_lexer = new ABC_lexer(print_parse_errors);
  ATerm result = an_ABC_lexer->parse(streams);
  delete an_ABC_lexer;
  return result;
}
...
{String}             { process_string(); return STRING; }
