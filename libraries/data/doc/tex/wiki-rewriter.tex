...
%TCIDATA{LastRevised=Monday, October 26, 2009 17:03:40}
...
2) & l\notin \mathbb{V} \\ 
3) & var(l)\cup var(c)\subseteq var(r)%
\end{array}%
\end{equation*}
...
\begin{problem}
How can the construction of match trees be extended to rewrite rules with
applicative terms at the left hand side?
\end{problem}
...
Let $M_{f}$ be a match tree corresponding to rewrite rules with head symbol $%
f$. We define the function $rewr_{M}$ as:%
\begin{equation*}
\begin{array}{lll}
rewr_{M}(x,\sigma ) & = & \sigma (x) \\ 
rewr_{M}(f,\sigma ) & = & M_{f}([f],\sigma ) \\ 
rewr_{M}(f(t_{1},\cdots ,t_{n}),\sigma ) & = & M_{f}([f(t_{1},\cdots
,t_{n})],\sigma ) \\ 
...
\begin{equation*}
\begin{array}{llll}
clean(T) & = & T^{\prime } & \text{if }\left\langle T^{\prime
},V\right\rangle =\chi (T) \\ 
...
\begin{problem}
How can the definition of $rewr_{I}$ be extended to applicative terms (\ref%
{eq:applicative_terms})?
\end{problem}
