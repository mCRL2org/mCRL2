...
inline sort_expression_list insert_unique(const sort_expression_list &list, const sort_expression &el)
{
                      ...
                      bool warn_upcasting)
{
  ...
  if (data::is_unknown_sort(NeededType))
  {
    return Type;
  }
  ...
  if (data::is_multiple_possible_sorts(NeededType))
  {
    multiple_possible_sorts mps(NeededType);
  ...
  if (warn_upcasting && data::is_function_symbol(Par) && gsIsNumericString(aterm_cast<aterm_appl>((Par)[0]).function().name()))
  {
                        ...
                        std::string previous_reason)
{
  ...
  if (is_multiple_possible_sorts(CandidateType))
  {
    CandidateList=aterm_cast<multiple_possible_sorts>(CandidateType).sorts();
  }
  else
  {
  ...
  while (true)
  {
    ...
    for (sort_expression_list l=CurrentCandidateList; !l.empty(); l=l.tail())
    {
      ...
      if (!List.empty())
      {
        ...
        NewCurrentCandidateList=core::insert_unique(NewCurrentCandidateList,List);
      }
      else
      {
...
bool mcrl2::data::data_type_checker::UnSet(sort_expression PosType, sort_expression &result) 
{
  ...
  {
    const multiple_possible_sorts mps=aterm_cast<multiple_possible_sorts>(PosType);
...
bool mcrl2::data::data_type_checker::UnArrowProd(sort_expression_list ArgTypes, sort_expression PosType, sort_expression &result)
{
  ...
  if (is_multiple_possible_sorts(PosType))
  {
    ...
    for (sort_expression_list PosTypes=mps.sorts(); !PosTypes.empty(); PosTypes=PosTypes.tail())
    {
      ...
      NewPosTypes=core::insert_unique(NewPosTypes,NewPosType);
    }
    NewPosTypes=reverse(NewPosTypes);
    result=multiple_possible_sorts(sort_expression_list(NewPosTypes));
    return true;
...
bool mcrl2::data::data_type_checker::UnifyMinType(const sort_expression &Type1, const sort_expression &Type2, sort_expression &result)
{
  ...
  if (is_multiple_possible_sorts(result))
  {
    result=aterm_cast<multiple_possible_sorts>(result).sorts().front();
  }
...
bool mcrl2::data::data_type_checker::MatchListSetBagOpIn(const sort_expression &type, sort_expression &result)
{
  ...
  result=function_sort(make_list<sort_expression>(Arg,Arg2.set_argument(Arg,1)),sort_bool::bool_());
  return true;
}
...
sort_expression mcrl2::data::data_type_checker::ExpandNumTypesUp(sort_expression Type)
{
  ...
  if (is_function_sort(Type))
  {
    ...
    sort_expression ResultType=aterm_cast<sort_expression>(Type[1]);
  ...
  const bool print_cast_error)
{
  ...
  if (is_identifier(DataTerm)||data::is_function_symbol(DataTerm))
  {
      ...
      else 
      {
        ...
        else
        {
          ...
          if (j!=system_constants.end())
          {
            ...
            {
              DataTerm=data::function_symbol(Name,data::unknown_sort());
...

    {
      ...
      for (; !ParList.empty(); ParList=ParList.tail())
      {
        ...
        {
          NewParList=aterm_cast<sort_expression_list>(core::insert_unique(NewParList,result));
        }
      }
      ...
      if (NewParList.empty())
      {
        ...
        {
          NewParList=make_list<sort_expression>(core::MinType(NewParList));
        }
      }
    ...
    if (ParList.size()==1)
    {
      ...
      if (core::HasUnknown(Type) && data::is_function_symbol(DataTerm))
      {
        sort_expression new_type;
      ...
      if (data::detail::if_symbol()==(DataTerm)[0])
      {
      ...
      if (data::detail::equal_symbol()==DataTerm[0]
          || data::detail::not_equal_symbol()==DataTerm[0]
          || data::detail::less_symbol()==DataTerm[0]
          || data::detail::less_equal_symbol()==DataTerm[0]
          || data::detail::greater_symbol()==DataTerm[0]
          || data::detail::greater_equal_symbol()==DataTerm[0]
         )
      {
      ...
      if (sort_list::cons_name()==DataTerm[0])
      {
      ...
      if (sort_list::snoc_name()==DataTerm[0])
      {
        mCRL2log(debug) << "Doing <| matching Type " << pp(Type) << ", PosType " << pp(PosType) << "" << std::endl;
        sort_expression NewType;
      ...
      if (sort_list::concat_name()==DataTerm[0])
      {
        mCRL2log(debug) << "Doing ++ matching Type " << pp(Type) << ", PosType " << pp(PosType) << "" << std::endl;
        sort_expression NewType;
      ...
      if (sort_list::element_at_name()==DataTerm[0])
      {
      ...
      if (sort_list::head_name()==DataTerm[0]||
          sort_list::rhead_name()==DataTerm[0])
      {
      ...
      if (sort_list::tail_name()==DataTerm[0]||
          sort_list::rtail_name()==DataTerm[0])
      {
      ...
      if (sort_bag::set2bag_name()==DataTerm[0])
      {
      ...
      if (sort_list::in_name()==DataTerm[0])
      {
      ...
      if (sort_set::union_name()==DataTerm[0]||
          sort_set::difference_name()==DataTerm[0]||
          sort_set::intersection_name()==DataTerm[0])
      {
      ...
      if (sort_set::complement_name()==DataTerm[0])
      {
      ...
      if (sort_bag::bag2set_name()==DataTerm[0])
      {
      ...
      if (sort_bag::count_name()==DataTerm[0])
      {
      ...
      if (data::function_update_name()==DataTerm[0])
      {
  ...
  const bool print_cast_error)
{
  ...
  if (is_abstraction(DataTerm))
  {
    ...
    if (is_lambda_binder(BindingOperator))
    {
      ...
      sort_expression_list ArgTypes=core::GetVarTypes(VarList);
      sort_expression NewType;
...
// aterm_list WhereList=aterm_cast<aterm_list>((DataTerm)[1]); !WhereList.empty(); WhereList=WhereList.tail())
    {
      ...
      if (is_variable(aterm_cast<aterm_appl>(WhereElem[0])))
      {
  ...
  if (is_application(DataTerm))
  {
    ...
    if (data::is_function_symbol(Arg0) || is_identifier(Arg0))
    {
    ...
    try
    {
      NewType=TraverseVarConsTypeDN(DeclaredVars,AllowedVars,
                      Data,
                      data::unknown_sort() /* function_sort(ArgumentTypes,PosType) */,
  ...
  if (is_identifier(DataTerm)||data::is_function_symbol(DataTerm)||is_variable(DataTerm))
  {
    core::identifier_string Name=aterm_cast<core::identifier_string>((DataTerm)[0]);
                 ...
                 sort_expression &result)
{
  ...
  if (is_container_sort(Type))
  {
    sort_expression ConsType = aterm_cast<sort_expression>(Type[0]);
    if (is_list_container(ConsType))
    {
      ...
      sort_expression Res;
      if (!TypeMatchA(aterm_cast<sort_expression>(Type[1]),aterm_cast<sort_expression>(PosType[1]),Res))
      {
        return false;
      }
...
bool mcrl2::data::data_type_checker::IsTypeAllowedA(const sort_expression &Type, const sort_expression &PosType)
{
  ...
  if (is_multiple_possible_sorts(PosType))
  {
    return InTypesA(Type,aterm_cast<const multiple_possible_sorts>(PosType).sorts());
  }
...
static sort_expression replace_possible_sorts(const sort_expression &Type)
{
  if (is_multiple_possible_sorts(data::sort_expression(Type)))
  {
    return aterm_cast<multiple_possible_sorts>(Type).sorts().front(); // get the first element of the possible sorts.
  }
  if (data::is_unknown_sort(data::sort_expression(Type)))
  {
    return data::unknown_sort();
  ...
  if (is_function_sort(Type))
  {
    ...
    for (aterm_list TypeList=aterm_cast<aterm_list>(Type[0]); !TypeList.empty(); TypeList=TypeList.tail())
    {
      NewTypeList.push_front(replace_possible_sorts(TypeList.front()));
    }
    sort_expression ResultType=aterm_cast<sort_expression>(Type[1]);
...
static bool HasUnknown(const sort_expression &Type)
{
  ...
  if (is_container_sort(Type))
  {
    return HasUnknown(aterm_cast<sort_expression>(Type[1]));
  }
  if (is_structured_sort(Type))
  {
    return false;
