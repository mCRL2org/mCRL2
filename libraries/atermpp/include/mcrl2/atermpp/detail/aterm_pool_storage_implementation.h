...
void ATERM_POOL_STORAGE::print_performance_stats(const char* identifier) const
{
  ...
  {
    mCRL2log(mcrl2::log::log_level_t::info) << "g_term_pool(" << identifier << ") hashtable:\n";
    print_performance_statistics(m_term_set);
  }

  if (EnableGarbageCollectionMetrics && m_erasedBlocks > 0)
  {
    mCRL2log(mcrl2::log::log_level_t::info) << "g_term_pool(" << identifier << "): Consolidate removed " << m_erasedBlocks << " blocks.\n";
  }

  if (EnableCreationMetrics)
  {
    mCRL2log(mcrl2::log::log_level_t::info) << "g_term_pool(" << identifier << "): emplace() " << m_term_metric.message() << ".\n";
  }
}
