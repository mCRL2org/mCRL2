...
static int IndexOf(ATermList ls, const int k, int i) {
...
static ATermList GetLabels(INTERVAL *p)
{
  ...
  ATermList result = branching?ATmakeList1((ATerm) ATmakeInt(label_tau)):ATempty;
...
static void AddBlock(const int b) {
  int i = 0;
  for (;i<blocks.pt;i++) {
...
static void Unstable(const int a, INTERVAL *p)
{
...
{
  int left = p->left, right = p->right, left_bound_p2;
  left_bound_p2 = partition(left, right - 1);
  if (left_bound_p2 == left) {
    for (int i=left;i<right;++i) mark[s[i]] = ATfalse;
    return ATfalse;
  }
...
static ATbool isParentOf(const int parent, int b) {
...
static void PushUnstableMark(const int action, const int splitter, const int parent, INTERVAL *p)
{
...
static ATbool Refine(const int action, const int splitter)
{
...
static ATbool CheckUnstableBlock(const int splitter)
...
static ATbool CompareCheckUnstableBlock(int splitter, int init1,
                                        int init2, ATbool *different)
/* for all labels while p is not splitted */
{
  INTERVAL *p = Pi+ splitter;
  ATermList labels = GetLabels(p);
  ATbool result = ATfalse;
  offset = second_lts_states_offset;
  for (;!ATisEmpty(labels) && !isRemoved(p);
  labels = ATgetNext(labels))
  {
    int a = ATgetInt((ATermInt) ATgetFirst(labels));
...
static ATbool CompareKernel(int init1,int init2,
                            ATbool *different)
{
  ATbool result = ATfalse;
  Pi_pt = n_partitions;
