...
public:
	...
	// constructor
	state_T(): id(state_index++), block(NULL), inert_cnt(0), priority(0), constln_cnt(NULL), coconstln_cnt(NULL), type(STATE), is_in_L_detect1(false), is_in_P_detect2(false), is_in_Lp_detect2(false) {Ttgt = new std::vector <transition_T*>; Tsrc = new std::vector <transition_T*>;}
	// destructor
	~state_T() {deleteobject(Ttgt); deleteobject(Tsrc);}
};
...
struct constellation_T
{
	...
	// constructor
	constellation_T(): id(max_const_index++), size(0), type(TRIVIAL) {blocks = new sized_forward_list < block_T >;}
	// destructor
  ...
  private:
...

    {
      ...
      unordered_map < Key, state_type, KeyHasher > extra_kripke_states;
			...
			constellation_T* C = new constellation_T;
				...
				for (auto const_it = non_trivial_constlns.begin(); const_it != non_trivial_constlns.end(); ++const_it) {
					...
					for (auto bit = setB->blocks->begin(); bit != setB->blocks->end(); ++bit) {
						...
						if (B->btm_states->size()+B->non_btm_states->size() <= (setB->size)/2) {
							...
							// 5.2.1
							// 5.2.1.a
							constellation_T* setC = new constellation_T;
