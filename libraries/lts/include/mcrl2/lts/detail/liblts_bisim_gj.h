...
using outgoing_transitions_const_it =
                        fixed_vector<outgoing_transition_type>::const_iterator;
...
/// default counter value if the counter field of a state is not in use
/// currently
...
struct outgoing_transition_type
{
  ...
  /// this pointer is used to find transitions with the same source state,
  /// action label, and target constellation
  /// (Transitions are grouped according to these in `m_outgoing_transitions`.)
  /// For most transitions, it points to the last transition with the same
  /// source state, action label, and target constellation;
  /// but if this transition is the last one in the group,
  /// `start_same_saC` points to the first transition in the group.
...
/// a vector with an additional (internal) field to indicate how much work has
/// been done already on it.
...
struct block_type
{
  ...
  union btc_R
  {
    ...
    /// the end of the list.
    linked_list<BLC_indicators> to_constellation;                               static_assert(std::is_trivially_destructible_v<linked_list<BLC_indicators>>);
      ...
      return result;
                                                                                        ...
                                                                                        m_states_in_blocks.data_end()!=si; si=si->ref_state->block->end_states)
                                                                                    {
                                                                                                   ...
                                                                                                   ind!=b.block.to_constellation.end(); ++ind)
                                                                                      {
                                                                                        ...
                                                                                        if (!ind->is_stable())
                                                                                        {
                                                                                          ...
                                                                                          mCRL2log(log::log_level_t::debug) << ind->debug_id(*this) << " contains " << std::distance(ind->start_marked_BLC, ind->end_same_BLC) << " marked transitions.\n";
                                                                                        ...
                                                                                        if (b.contains_new_bottom_states)
                                                                                        {
                                                                                          ...
                                                                                          {
                                                                                            mCRL2log(log::log_level_t::debug) << "  This is ok because " << b.debug_id(*this) << " contains new bottom states.\n";
                                                                                                                                             ...
                                                                                                                                             new_constellation)
                                                                                                  {
                                                                                                    if (calM_elt->first<=main_splitter->start_same_BLC && main_splitter->end_same_BLC<=calM_elt->second)
                                                                                                    {
                                                                                                      assert(new_constellation==
                                                                                                                  m_states[main_t.to()].block->c.onstellation);
                                                                                                      mCRL2log(log::log_level_t::debug) << "  This is ok because the BLC set (" << b.debug_id(*this) << " -" << m_aut.action_label(first_t.label()) << "-> " << old_constellation->debug_id(*this) << ") is soon going to be a co-splitter.\n";
                                                                                                      eventual_instability_is_ok = true;
                                                                                                      eventual_marking_is_ok = true;
                                                                                                    }
                                                                                                  }
                                                                                        ...
                                                                                        if (1>=number_of_states_in_block(b))
                                                                                        {
                                                                                          ...
                                                                                          {
                                                                                            mCRL2log(log::log_level_t::debug) << "  (This is ok because the source block contains only 1 state.)\n";
                                                                                            eventual_marking_is_ok = true;
                                                                                          }
                                                                                        }
                                                                                                             ...
                                                                                                             const bool initialisation=false) const
                                                                                  {
                                                                                    ...
                                                                                    for(state_index si=0; si<m_aut.num_states(); ++si)
                                                                                    {
                                                                                                   ...
                                                                                                   ++it)
                                                                                      {
                                                                                        ...
                                                                                        if (start_same_saC_valid)
                                                                                        {
                                                                                          ...
                                                                                          {
                                                                                            mCRL2log(log::log_level_t::debug) << " -- error: different target " << m_states[t.to()].block->c.onstellation->debug_id(*this);
                                                                                          }
                                                                                      ...
                                                                                      mCRL2log(log::log_level_t::debug) << "  Ref states in blocks: " << std::distance<fixed_vector<state_type_gj>::const_iterator>(m_states.cbegin(), m_states[si].ref_states_in_blocks->ref_state) << ". Must be " << si <<".\n";
                                                                                      mCRL2log(log::log_level_t::debug) << "---------------------------------------------------\n";
                                                                                    }
                                                                                        ...
                                                                                        m_states_in_blocks.data_end()!=si; si=si->ref_state->block->end_states)
                                                                                    {
                                                                                      ...
                                                                                      mCRL2log(log::log_level_t::debug) << "  Block " << bi;
                                                                                      if (!initialisation) {
                                                                                        mCRL2log(log::log_level_t::debug) << " (" << bi->c.onstellation->debug_id(*this) << ')';
                                                                                      }
  ...
  private:
                    ...
                    constellation_type* const new_constellation)
    {
      ...
      if (0==bottom_size(ReachAlw))
      {
        ...
        ++no_of_finished_searches;
        status[ReachAlw]=finished;                                              assert(aborted!=status_NewBotSt);
      ...
      {                                                                         assert(2>=no_of_finished_searches);
        ...
        #define new_start_bottom_states(idx) (                                  assert(1<=(idx)), assert((idx)<=3), \
                                     new_start_bottom_states_plus_one[(idx)-1])
        #define new_end_bottom_states(idx) (                                    assert(1<=(idx)), assert((idx)<=2), \
                                       new_end_bottom_states_plus_one[(idx)-1])
        #define new_end_bottom_states_NewBotSt \
                                          (new_start_bottom_states_plus_one[2])
                                ...
                                no_of_running_searches; ++current_search_index)
        {
              ...
              non_bottom_states[current_search].todo_is_empty())
          {
            ...
            if (3>no_of_finished_searches)
            {
              ...
              for (const state_in_block_pointer& st:
                                   potential_non_bottom_states[current_search])
        ...
        {                                                                       assert(current_source_iter_NewBotSt<current_source_iter_end_NewBotSt);
              ...
              !(m_preserve_divergence && tr.from()==tr.to()))
          {
            ...
            {                                                                   assert(!non_bottom_states_NewBotSt.find(src));
              /* Algorithm 2, Line 2.12                                      */ assert(aborted!=status_NewBotSt);
            ...
            do
            {
              ...
              else
              {
                ...
                {                                                               assert(!non_bottom_states[ReachAlw].find(src));
                  ...
                  /* AvoidLrg-coroutine could add state src erroneously.     */ assert(aborted!=status_NewBotSt);
                  abort_if_non_bottom_size_too_large_NewBotSt(0);
                  break;
...
// 2. Administration: Mark all transitions out of (new) bottom states
    ...
    void create_initial_partition()
    {
      ...
      // mCRL2log(log::log_level_t::verbose) << "Start refining in the initialisation WITHOUT BLC sets\n";
...
// =================================================================================================================================
                    ...
                    j=i->ref_state->start_incoming_transitions; j!=end_it; ++j)
          {
            ...
            if (update_the_doubly_linked_list_LBC_new_constellation
                ...
                (!is_inert_during_init(t) ||
                 index_block_B!=m_states[t.from()].block))
            {
  ...
  public:
    ...
    ///                            tau loops on states.
    bisim_partitioner_gj(LTS_TYPE& aut, const bool branching = false,
