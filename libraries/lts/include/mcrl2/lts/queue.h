...
namespace mcrl2
{
  namespace lts
  {
    ...
    class queue
    {
      ...
      public:
        queue() :
          queue_get(NULL), queue_put(NULL), queue_size(0), queue_size_max(
              UINT_MAX), queue_get_pos(0), queue_get_count(0), queue_put_count(
              0), queue_put_count_extra(0), queue_size_fixed(false)
        {
        }
