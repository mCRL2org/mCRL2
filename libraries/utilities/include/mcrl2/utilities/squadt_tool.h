...
# endif
          ...
          void update_configuration(tipi::configuration& c) {
            if (!c.option_exists("rewrite_strategy")) {
              c.add_option("rewrite_strategy");
            }

            c.get_option("rewrite_strategy").set_argument_value< 0 >(this->rewrite_strategy());

            squadt_tool< Tool >::update_configuration(c);
          }

          /// \brief Adds rewrite strategy to the configuration or vice versa if the strategy is part of the configuration
          void synchronise_with_configuration(tipi::configuration& c) {
            if (c.option_exists("rewrite_strategy")) {
              this->m_rewrite_strategy = c.get_option_argument< mcrl2::new_data::rewriter::strategy >("m_rewrite_strategy", 0);
            }
            else {
              c.add_option("rewrite_strategy").set_argument_value< 0 >(this->rewrite_strategy());
            }

            this->synchronise_with_configuration(c);
          }
        ...
        public:
          ...
          squadt_tool(const std::string& name,
                        ...
                        const std::string& what_is,
                        const std::string& tool_description)
            : rewriter_tool< squadt_tool< Tool > >(name, author, what_is, tool_description)
          {}
      };
