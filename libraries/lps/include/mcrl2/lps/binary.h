...
protected:
    ...
    const data::data_expression_vector& enumerated_elements)
  {
    ...
    else
    {
      ...
      result = if_(condition, make_if_tree(new_parameters, right_list), make_if_tree(new_parameters, left_list));
    }

    return result;
  }

  /// \brief Build a unary if-then-else tree of enumerated elements in terms
  ///        of new parameters.
  /// \return unary if then else tree from enumerated_elements in terms of new_parameters
  data::data_expression make_linear_substitution(data::variable_vector new_parameters,
    const data::data_expression_vector& enumerated_elements)
  {
    data::data_expression result;
    mCRL2log(log::debug) << "new elements " << data::pp(new_parameters) << std::endl;
    mCRL2log(log::debug) << "enumerated elements " << data::pp(enumerated_elements) << std::endl;
    mCRL2log(log::debug) << "result of make linear substitution: " << result << std::endl;
    ...
    {
      data::data_expression head = enumerated_elements.back();
      data::data_expression_vector tail = enumerated_elements;
      tail.pop_back();

      data::data_expression condition = new_parameters.back();
      new_parameters.pop_back();
      result = if_(condition, head, make_linear_substitution(new_parameters, tail));
    }
  ...
  std::set<data::variable> select_parameters(const std::string parameter_selection) const
  {
    ...
    selected_params.remove_if(
      [&](const data::variable& v)
      {
        ...
        {
          mCRL2log(log::info) << "Not selecting " << v << ":" << v.sort()
  ...
  void replace_enumerated_parameters(const std::set<data::variable>& selected_params)
  {
    ...
    data::enumerator_algorithm<> enumerator(m_rewriter,
      m_spec.data(),
      m_rewriter,
      m_id_generator,
      accept_solutions_with_variables);
    ...
    for (const data::variable& par: process_parameters)
    {
      if (selected_params.find(par) != selected_params.end())
      {
        ...
        std::size_t n = nr_of_booleans_for_elements(enumerated_elements.size(), m_unary_option);
        ...
        // n = new_pars.size() && new_pars.size() = ceil(log_2(j)) && new_pars.size() =
        // ceil(log_2(enumerated_elements.size()))

        mCRL2log(log::verbose) << "Parameter " << data::pp(par) << ":" << data::pp(par.sort())
                               << " has been replaced by " << new_pars.size() << " parameter(s) " << data::pp(new_pars)
                               << " of sort Bool" << std::endl;

        // Store new parameters in a hastable
        m_new_parameters[par] = new_pars;

        // Check which encoding is needed to fill in the substitution
        if (m_unary_option)
        {
        ...
        {
          mCRL2log(log::debug) << "Using binary encoding" << std::endl;
          m_substitution[par] = make_if_tree(new_pars, enumerated_elements);
        }
      }
      else
      {
  ...
  data::data_expression_list replace_enumerated_parameters_in_initial_expressions(const data::variable_list& vl,
    const data::data_expression_list& el)
  {
    ...
    // binding variables.
    const data::data_expression_list el_ = data::replace_variables(el, m_substitution);

    data::data_expression_vector result;
    data::variable_list::const_iterator i = vl.begin();

    mCRL2log(log::debug) << "The replace variables el_ are " << data::pp(el_) << std::endl;
    for (const data::data_expression& a: el_)
    {
      const data::variable par = *i;
      i++;
      if (m_new_parameters.find(par)
          == m_new_parameters.end()) // This parameter is not replaced by a boolean parameters.
      {
        result.push_back(a);
      }
      else
      {
        ...
        data::data_expression_vector elements = m_enumerated_elements[par]; // The data values

        mCRL2log(log::debug) << "Found " << new_parameters.size() << " new parameter(s) for parameter " << data::pp(par)
                             << std::endl;

        if (m_unary_option)
        {
          // Go over all the values and put the corresponding position to false
          data::data_expression_vector::iterator k = elements.begin();
          while (k != elements.end())
          {
            result.push_back(data::equal_to(a, *k++));
          }
        }
        else
        {
          for (std::size_t j = 0; j < new_parameters.size(); ++j)
          {
            ...
            result.push_back(data::lazy::join_and(disjuncts.begin(), disjuncts.end()));
          }
        }
      }
    }

    mCRL2log(log::debug) << "Replaced expression(s) " << data::pp(el_) << " in the initial state with expression(s) "
                         << data::pp(result) << std::endl;
  ...
  data::assignment_list replace_enumerated_parameters_in_assignments(data::assignment_list v)
  {
    ...
    // Note that this operation is safe because the generated fresh variables can not clash with other
    // binding variables.
    v = data::replace_variables(v, m_substitution);
    ...
    for (const data::assignment& a: v)
    {
      ...
      else
      {
        ...
        mCRL2log(log::debug) << "Found " << new_parameters.size() << " new parameter(s) for parameter "
        ...
        if (m_unary_option)
        {
          ...
          for (std::size_t j = 0; j < new_parameters.size(); ++j)
          {
            ...
            result.emplace_back(new_parameters[j], data::lazy::join_and(disjuncts.begin(), disjuncts.end()));
          }
        }
        else
        {
    ...
    mCRL2log(log::debug) << "Replaced assignment(s) " << data::pp(v) << " with assignment(s) " << data::pp(result)
                         << std::endl;
  ...
  stochastic_process_initializer update_initial_process(const data::variable_list& parameters,
    const stochastic_process_initializer& init)
  {
    ...
    const stochastic_distribution dist
      = lps::replace_variables_capture_avoiding(init.distribution(), d, m_substitution, m_generator);
...
public:
  ...
  void run()
  {
    ...
    mCRL2log(log::debug) << "Using flags:" << m_parameter_selection << std::endl;
    const std::set<data::variable> to_replace = select_parameters(m_parameter_selection);

    // Calculate how to replace parameters, this fills in m_substitution
