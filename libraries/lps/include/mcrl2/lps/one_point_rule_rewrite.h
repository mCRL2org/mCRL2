...
template <typename T>
void one_point_rule_rewrite(T& x, typename std::enable_if<!std::is_base_of<atermpp::aterm_core, T>::value>::type* = nullptr)
{
...
template <typename T>
T one_point_rule_rewrite(const T& x, typename std::enable_if<std::is_base_of<atermpp::aterm_core, T>::value>::type* = nullptr)
{
  T result;
  detail::one_point_rule_rewrite_builder f;
  f.apply(result, x);
