...
//
/// \file mcrl2/lps/detail/lps_variable_traverser.h
...
namespace detail {
  ...
  struct lps_variable_traverser
  {
    OutIter dest;
  
    lps_variable_traverser(OutIter dest_)
      : dest(dest_)
    {}
    ...
    {                                         
      dest = data::traverse_variables(d, dest);
    } 
    ...
    {
      dest = data::traverse_variables(a, dest);
    } 
    ...
    {
      traverse_container(i.free_variables());
  ...
  /// \brief Utility function to create an lps_variable_traverser.
  template <typename OutIter>    
  lps_variable_traverser<OutIter> make_lps_variable_traverser(OutIter dest)        
  {
    return lps_variable_traverser<OutIter>(dest);
  } 
...
#endif // MCRL2_LPS_DETAIL_LPS_VARIABLE_TRAVERSER_H
