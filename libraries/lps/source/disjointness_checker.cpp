...
// Auxiliary functions ----------------------------------------------------------------------------
  ...
  bool disjoint_sets(bool* a_set_1, bool* a_set_2, size_t a_number_of_elements) {
    for (size_t i = 0; i < a_number_of_elements; i++) {
...
// Class Disjointness_Checker ---------------------------------------------------------------------
  // Class Disjointness_Checker - Functions declared private --------------------------------------
    ...
    void Disjointness_Checker::process_data_expression(size_t a_summand_number, ATermAppl a_expression) {
      if (gsIsDataVarId(a_expression)) {
        size_t v_variable_index = ATindexedSetGetIndex(f_parameter_set, (ATerm) a_expression);
        if (v_variable_index != ATERM_NON_EXISTING_POSITION) {
    ...
    void Disjointness_Checker::process_multi_action(size_t a_summand_number, ATermAppl v_multi_action) {
    ...
    void Disjointness_Checker::process_summand(size_t a_summand_number, ATermAppl a_summand) {
  ...
  // Class Disjointness_Checker - Functions declared public ---------------------------------------
    ...
    bool Disjointness_Checker::disjoint(size_t a_summand_number_1, size_t a_summand_number_2) {
      if ((a_summand_number_1 <= f_number_of_summands) && (a_summand_number_2 <= f_number_of_summands)) {
        ...
        throw mcrl2::runtime_error("There is no summand with this number.\n");
      }
    }
