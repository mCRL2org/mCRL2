...
{
  if (ATindexOf(l,m,0)==ATERM_NON_EXISTING_POSITION)
  {
    return ATinsert(l,m);
  }
  else
...
static ATermList list_minus_ignore_type(ATermList l, ATermList m)
{
  ...
  for (; !ATisEmpty(l); l=ATgetNext(l))
  {
    if (ATindexOf(m,(ATerm)ATAgetArgument(ATAgetFirst(l),0),0) == ATERM_NON_EXISTING_POSITION)
    {
      n = ATinsert(n,ATgetFirst(l));
    }
  }
...
static ATermList filter_block_list(ATermList l, ATermList H)
{
  ...
  for (; !ATisEmpty(l); l=ATgetNext(l))
  {
    ...
    {
      if (ATindexOf(ma,(ATerm)ATAgetFirst(tH),0) != ATERM_NON_EXISTING_POSITION)
      {
        b = false;
        break;
      }
...
static ATermList filter_hide_list(ATermList l, ATermList I)
{
  ...
  for (; !ATisEmpty(l); l=ATgetNext(l))
  {
    ...
    for (ATermList ma=ATLgetFirst(l); !ATisEmpty(ma); ma=ATgetNext(ma))
      if (ATindexOf(I,(ATerm)untypeA(ATAgetFirst(ma)),0)== ATERM_NON_EXISTING_POSITION)
      {
        new_ma=ATinsert(new_ma,ATgetFirst(ma));
      }
    if (ATgetLength(new_ma) && ATindexOf(m,(ATerm)new_ma,0)==ATERM_NON_EXISTING_POSITION)
    {
      m=ATinsert(m,(ATerm)ATreverse(new_ma));
    }
  }
  return ATreverse(m);
}
...
static ATermList optimize_allow_list(ATermList V, ATermList ul)
{
  ...
  for (; !ATisEmpty(V); V=ATgetNext(V))
  {
    ...
    if (ATindexOf(ul,(ATerm)ATLgetArgument(ma,0),0) != ATERM_NON_EXISTING_POSITION)
    {
...
static bool sub_multiaction(ATermList l, ATermList m)
{
  ...
  for (; !ATisEmpty(l); l=ATgetNext(l))
  {
    ...
    if (index == ATERM_NON_EXISTING_POSITION)
    {
      return false;
    }
...
static bool sub_multiaction_list(ATermList MAct, ATermList MActL)
{
  ...
  for (; !ATisEmpty(MActL); MActL=ATgetNext(MActL))
    if (sub_multiaction(MAct,ATLgetFirst(MActL)))
    {
      return true;
    }
  return false;
...
static ATermList apply_hide(ATermList I, ATermList MAct)
{
  ...
  for (; !ATisEmpty(MAct); MAct=ATgetNext(MAct))
  {
    ...
    if (ATindexOf(I,(ATerm)Act,0)== ATERM_NON_EXISTING_POSITION)
    {
...
static bool can_split_comm(ATermList C)
{
  ...
  {
    if (ATindexOf(rhs,ATgetFirst(lhs),0) != ATERM_NON_EXISTING_POSITION)
    {
      b = false;
      break;
    }
...
static ATermList apply_comms(ATermList l, ATermList C, ATermList lhs)
{
  ...
  if (ATisEqual(l,ll))
  {
    return ATmakeList1((ATerm)l);  //C does not apply
  }
  if (!ATisEmpty(ll))
  {
  ...
  while (ATgetLength(r) > 0)
  {
    ...
    for (ATermList tC=C; !ATisEmpty(tC); tC=ATgetNext(tC))
    {
      ...
      if (ATindexOf(c,ATgetArgument(a,0),0) !=ATERM_NON_EXISTING_POSITION)
      {
...
static ATermList extend_allow_comm(ATermList V, ATermList C)
{
  ...
  for (ATermList tV=V; !ATisEmpty(tV); tV=ATgetNext(tV))
  {
    ...
    for (; !ATisEmpty(v); v=ATgetNext(v))
    {
      ...
      if (r)
      {
        r=merge_list(r,ATmakeList1((ATerm)ATmakeList1((ATerm)a)));
      }
      else
      {
        r=ATmakeList1((ATerm)ATmakeList1((ATerm)a));
      }
      res=sync_list(res,r);
    }
...
static ATermAppl PushBlock(ATermList H, ATermAppl a)
{
  ...
  else if (gsIsComm(a))
  {
    ...
    {
      if ((ATindexOf(lhs,ATgetFirst(H),0) != ATERM_NON_EXISTING_POSITION) || (ATindexOf(rhs,ATgetFirst(H),0) != ATERM_NON_EXISTING_POSITION))
      {
  ...
  else if (gsIsSync(a) || gsIsMerge(a) || gsIsLMerge(a))
  {
...

    {
...

      {
        ...
        if (!lp)
        {
          lp=gsaGetAlpha(p,max_len,allowed);
        }
        if (!lq)
        {
          lq=gsaGetAlpha(q,max_len,allowed);
        }
...
static ATermAppl PushComm(ATermList C, ATermAppl a)
{
  ...
  {
    a = gsApplyAlpha(a);
    if (!gsIsAllow(a)) /* call ourselves recursively */
    {
      return PushComm(C,a);
    }
    ATermList l = ATLtableGet(alphas,(ATerm) a);
...
static ATermList gsaGetAlpha(ATermAppl a, size_t length, ATermList allowed, ATermList ignore)
{
  ...
  if (all_stable)
  {
    ...
    ATermAppl p=a;
    if (gsIsAction(a) || gsIsProcess(a) || gsIsProcessAssignment(a))
    {
      p=ATAgetArgument(a,0);
    }
    l=ATLtableGet(alphas,(ATerm) p);
    if (l)
    {
           ...
           || gsIsIfThen(a) || gsIsIfThenElse(a) || gsIsSync(a) || gsIsMerge(a) || gsIsLMerge(a) || gsIsBInit(a))
  {
    ...
    if (gsIsIfThen(a) || gsIsSum(a) || gsIsAtTime(a) || gsIsBInit(a))
    {
    ...
    if (args==2)
    {
      l2 = gsaGetAlpha(ATAgetArgument(a,ia2),length,allowed);
    }
...
static ATermAppl gsApplyAlpha(ATermAppl a)
{
           ...
           || gsIsIfThen(a) || gsIsIfThenElse(a) || gsIsSync(a) || gsIsMerge(a) || gsIsLMerge(a) || gsIsBInit(a))
  {
    ...
    if (args==2)
    {
      q = gsApplyAlpha(ATAgetArgument(a,ia2));
    }
...
ATermList gsaGetDeps(ATermAppl a)
{
           ...
           || gsIsIfThen(a) || gsIsIfThenElse(a) || gsIsSync(a) || gsIsMerge(a) || gsIsLMerge(a) || gsIsBInit(a))
  {
    ...
    if (args==2)
    {
      r = merge_list(r,gsaGetDeps(ATAgetArgument(a,ia2)));
    }
    return r;
  }
  assert(0);
...
ATermAppl gsaGetProp(ATermAppl a, ATermAppl context)
{
           ...
           || gsIsIfThen(a) || gsIsIfThenElse(a) || gsIsBInit(a))
  {
    ...
    if (ATisEqual(p,mCRL_aterm))
    {
      r=mCRL_aterm;
    }
    if (args==2 && ATisEqual(gsaGetProp(ATAgetArgument(a,ia2),context),mCRL_aterm))
    {
      r=mCRL_aterm;
    }
  }
  ...
  else if (gsIsSync(a)||gsIsMerge(a)||gsIsLMerge(a))
  {
    ...
    if (ATindexOf(deps,(ATerm)context,0)!=ATERM_NON_EXISTING_POSITION)
    {
      r=mCRL_aterm;
    }
    else
...
ATermAppl gsaSubstNP(ATermTable subs_npCRL, ATermTable consts, ATermAppl a)
{
  ...
  else if (gsIsProcess(a) || gsIsProcessAssignment(a))
  {
    ...
    if (!l)
    {
      return a;  //not an npCRL process call.
    }
  ...
  process_expression& init)
{
  ...
  while (!stable)
  {
    ...
    if (todo)
    {
      todo=ATinsert(todo,(ATerm)INIT_KEY());
    }
    else
    {
      todo=ATmakeList1((ATerm)INIT_KEY());
    }
  ...
  for (; !ATisEmpty(todo); todo=ATgetNext(todo))
  {
    ...
    bool rec=(ATindexOf(dep,(ATerm)p,0)!=ATERM_NON_EXISTING_POSITION);
  ...
  for (; !ATisEmpty(todo); todo=ATgetNext(todo))
  {
    ...
    if (ATisEqual(ATAtableGet(props,(ATerm)p),npCRL))
    {
      ...
      if (P)
      {
        ...
        if (!data::sort_pos::is_pos(data::sort_expression(ATAgetArgument(FormPar1,1))))
        {
          goto nP_checked;
        }
        ...
        if (!(nParsP==nParsNP || nParsP+1==nParsNP))
        {
          goto nP_checked;
        }
        ...
        if (!ATisEqual(ATAgetArgument(body,0),static_cast<ATermAppl>(data::greater(data::variable(Varp, data::sort_pos::pos()), data::function_symbol(Number1, data::sort_pos::pos())))))
        {
          goto nP_checked;
        }
        ...
        if (!ATisEqual(P,ATAgetArgument(Right,0)))
        {
          goto nP_checked;
        }
        ...
        if (nParsP==nParsNP)
        {
          ...
          if (!ATisEqual(Number1,ATAgetArgument(ATAgetFirst(ActParsP),0)))
          {
            goto nP_checked;
          }
        ...
        if (!ATisEqual(ATgetNext(FormParsNP),ActParsP))
        {
          goto nP_checked;
        }
        ...
        if (!gsIsMerge(parallel))
        {
          goto nP_checked;
        }
        ATermAppl parallel_left=ATAgetArgument(parallel,0);
        if (!gsIsProcess(parallel_left))
        {
          goto nP_checked;
        }
        ATermAppl parallel_right=ATAgetArgument(parallel,1);
        if (!gsIsProcess(parallel_right))
        {
          goto nP_checked;
        }
        if (!ATisEqual(P,ATAgetArgument(parallel_left,0)))
        {
          goto nP_checked;
        }
        if (!ATisEqual(p,ATAgetArgument(parallel_right,0)))
        {
          goto nP_checked;
        }
        ...
        if (nParsP==nParsNP)
        {
          if (!ATisEqual(ATLgetArgument(parallel_left,1),FormParsNP))
          {
            goto nP_checked;
          }
        }
        ...
        {
          if (!ATisEqual(ATLgetArgument(parallel_left,1),ATgetNext(FormParsNP)))
          {
            goto nP_checked;
          }
        }
        ...
        if (!ATisEqual(ATgetNext(ParsRight),ATgetNext(FormParsNP)))
        {
          goto nP_checked;
        }
...
nP_checked:
  ...
  for (ATermList pr=todo; !ATisEmpty(pr); pr=ATgetNext(pr))
  {
    ...
    if (ATisEqual(ATAgetArgument(ATAtableGet(props,(ATerm)pn),0),mCRL_aterm))
    {
      continue;
    }
