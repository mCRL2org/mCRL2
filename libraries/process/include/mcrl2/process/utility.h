...
inline
void apply_comm_inverse(const std::vector<core::identifier_string>& alpha1, std::vector<core::identifier_string>& alpha2, std::size_t i, const std::map<core::identifier_string, core::identifier_string_list>& Cinverse, multi_action_name_set& result)
{
  if (i >= alpha1.size())
  {
...
multi_action_name_set apply_comm_inverse(const communication_expression_list& C, const multi_action_name_set& A)
{
  ...
  for (multi_action_name_set::const_iterator j = A.begin(); j != A.end(); ++j)
  {
    ...
    std::vector<core::identifier_string> alpha1; // elements of alpha that are present in Cinverse
    std::vector<core::identifier_string> alpha2; // elements of alpha that are not present in Cinverse
