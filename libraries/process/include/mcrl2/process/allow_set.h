...
allow_set rename_inverse(const rename_expression_list& R, const allow_set& x)
{
  ...
  mCRL2log(log::log_level_t::trace) << "rename_inverse(" << R << ", " << x << ") = " << result << std::endl;
  return result;
}
  ...
  const allow_set& x)
{
  ...
  mCRL2log(log::log_level_t::trace) << "comm_inverse(" << C << ", " << core::pp(action_names) << ", " << x << ") = " << result << std::endl;
  return result;
}
...
allow_set left_arrow(const allow_set& x, const multi_action_name_set& A)
{
  ...
  result.establish_invariant();
  mCRL2log(log::log_level_t::trace) << "left_arrow(" << x << ", " << process::pp(A) << ") = " << result << std::endl;
  return result;
}
...
allow_set subsets(const allow_set& x)
{
  ...
  {
    mCRL2log(log::log_level_t::debug) << "allow_set::subsets: skipped remove_subsets on a set of " << result.A.size() << " elements." << std::endl;
  }
  result.establish_invariant();
  return result;
}
