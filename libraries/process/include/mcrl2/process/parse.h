...
struct process_actions: public lps::action_actions
{
  ...
  process::process_expression parse_ProcExpr(const core::parse_node& node)
  {
    ...
    else if ((node.child_count() == 3) && (symbol_name(node.child(0)) == "DataExprUnit") && (symbol_name(node.child(1)) == "IfThen") && (symbol_name(node.child(2)) == "ProcExpr")) { return if_then_else(parse_DataExprUnit(node.child(0)), parse_ProcExpr(node.child(1).child(1)), parse_ProcExpr(node.child(2))); }
