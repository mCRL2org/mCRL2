...
namespace squadt {
  namespace GUI {
    ...
    void project::load_views() {
      wxTreeItemId root_item_id = file_view->GetRootItem();

      file_view->DeleteChildren(root_item_id);
      ...
      while (i.valid() && (*i)->number_of_inputs() == 0) {
        for (processor::output_object_iterator j = (*i)->get_outputs_iterator(); j.valid(); ++j) {
          wxTreeItemId item = file_view->AppendItem(root_item_id,
                      wxString(boost::filesystem::path((*j)->location).leaf().c_str(), wxConvLocal), 3);

          file_view->SetItemData(item, new node_data(*this, *j));

          file_view->EnsureVisible(item);
        }
      ...
      while (!id_stack.empty()) {
        ...
        for (wxTreeItemId j = file_view->GetFirstChild(c, cookie);
                                  j.IsOk(); j = file_view->GetNextChild(c, cookie)) {

          processor* t = static_cast < node_data* > (file_view->GetItemData(j))->target->generator;

          /* Look for inputs *i is generated by *t */
          for (project_manager::processor_iterator k = i; k.valid(); ++k) {
            for (processor::input_object_iterator l = (*k)->get_inputs_iterator(); l.valid(); ++l) {
              if (*l != 0 && (*l)->generator == t) {
                for (processor::output_object_iterator m = (*k)->get_outputs_iterator(); m.valid(); ++m) {
              
                  wxTreeItemId item = file_view->AppendItem(j,
                              wxString(boost::filesystem::path((*m)->location).leaf().c_str(), wxConvLocal), 0);
              
                  file_view->SetItemData(item, new node_data(*this, *m));
    ...
    void project::build() {
      ...
      file_view            = new wxTreeCtrl(this, wxID_ANY, wxDefaultPosition, wxDefaultSize,
                                                        wxTR_HAS_BUTTONS|wxTR_SINGLE|wxSUNKEN_BORDER);

      SetMinimumPaneSize(GetParent()->GetSize().GetWidth() / 2);

      SplitVertically(file_view, process_display_view);

      process_display_view->SetSizer(new wxBoxSizer(wxVERTICAL));
      process_display_view->SetScrollRate(10, 10);

      file_view->SetImageList(format_icon_list);
      file_view->AddRoot(wxT("Store"));

      /* Connect event handlers */
      file_view->Connect(wxEVT_COMMAND_TREE_ITEM_MENU, wxTreeEventHandler(project::on_tree_item_activate), 0, this);
      file_view->Connect(wxEVT_COMMAND_TREE_ITEM_ACTIVATED, wxTreeEventHandler(project::on_tree_item_activate), 0, this);
    ...
    void project::on_tree_item_activate(wxTreeEvent& e) {
      if (file_view->GetRootItem() != e.GetItem()) {
        spawn_context_menu(*(static_cast < node_data* > (file_view->GetItemData(e.GetItem()))->target));
      }
      else {
        ...
        if (dialog.ShowModal()) {
          ...
          wxTreeItemId i = file_view->AppendItem(e.GetItem(), wxString(dialog.get_name().c_str(), wxConvLocal), 3);
          ...
          file_view->SetItemData(i, new node_data(*this, *(p->get_outputs_iterator())));
          file_view->EnsureVisible(i);
        }
      }
    }
    ...
    void project::on_context_menu_select(wxCommandEvent& e) {
      wxTreeItemId                  s = file_view->GetSelection();
      processor::object_descriptor* t = reinterpret_cast < node_data* > (file_view->GetItemData(s))->target;
      ...
      switch (e.GetId()) {
        ...
        case cmID_DETAILS: {
            ...
            if (file_view->GetItemParent(s) == file_view->GetRootItem()) {
              dialog.set_name(file_view->GetItemText(s));
