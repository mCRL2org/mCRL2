...
#endif
static void implement_tree_aux(FILE *f, ATermAppl tree, int cur_arg, int parent, int level, int cnt, int d, int arity)
...
static void implement_tree(FILE *f, ATermAppl tree, int arity, int d, int opid)
{
#ifdef IT_DEBUG
...
{ ATermList l,m,n;
  ...
  ATprotectAFun(afunX);
  afunRe = ATmakeAFun("@@Re",2,ATfalse); // End of tree ( matching_rule, vars_in_rule )
  ATprotectAFun(afunRe);
  afunCRe = ATmakeAFun("@@CRe",4,ATfalse); // End of tree ( condition, matching_rule, vars_in_cdndition, vars_in_rule )
...
//      "    ATfprintf(stderr,\"FUNCTION %%i %%t\\n\",function_index,t); \n"
      ...
      "    }\n"
      "  } else {\n"
      "    ATerm a = RWapplySubstitution(ATgetArgument(t,0));\n"
      "    long function_index;\n"
      "    if ( ATisInt(ATgetArgument(a,0)) && (function_index = ATgetInt((ATermInt)ATgetArgument(t,0))) < %i )\n"
      "    {\n"
      "      return int2func[function_index](t);\n"
      "    } else {\n"
      "      int head_arity = isAppl(a)?ATgetArity(ATgetAFun((ATermAppl) a)):1;\n"
      "      int arity = ATgetArity(ATgetAFun(t))-1+head_arity;\n"
      "      ATerm args[arity];\n"
      "\n"
      "      args[0] = isAppl(a)?ATgetArgument(a,0):a;\n"
      "      for (int i=1; i<head_arity; i++)\n"
      "      {\n"
      "        args[i] = (ATerm) rewrite((ATermAppl) ATgetArgument(a,i));\n"
      "      }\n"
      "      for (int i=head_arity; i<arity; i++)\n"
      "      {\n"
      "        args[i] = (ATerm) rewrite((ATermAppl) ATgetArgument(t,i-head_arity+1));\n"
      "      }\n"
      "\n"
      "      return ATmakeApplArray(ATgetAFun(t),args);\n"
      "    }\n"
