...
lpssuminst = subprocess.run(
    ["lpssuminst", "-v"], input=mcrl22lps.stdout, stdout=subprocess.PIPE, check=True
)
lpsparunfold = subprocess.run(
    ["lpsparunfold", "-sBoard", "-n6"],
    input=lpssuminst.stdout,
    stdout=subprocess.PIPE,
    check=True,
)
lpsparunfold = subprocess.run(
    ["lpsparunfold", "-sRow", "-n8"],
    input=lpsparunfold.stdout,
    stdout=subprocess.PIPE,
    check=True,
)
lpsrewr = subprocess.run(
    ["lpsrewr", "-v"], input=lpsparunfold.stdout, stdout=subprocess.PIPE, check=True
)
subprocess.run(
    ["lpsconstelm", "-v", "-", "temp.lps"],
    input=lpsrewr.stdout,
    stdout=subprocess.PIPE,
    check=True,
)

lps2pbes = subprocess.run(
    ["lps2pbes", "-m", "-f", "red_wins.mcf", "temp.lps"],
    stdout=subprocess.PIPE,
    check=True,
)
pbesconstelm = subprocess.run(
    ["pbesconstelm", "-v", "-", "temp.red_wins.pbes"],
    input=lps2pbes.stdout,
    stdout=subprocess.PIPE,
    check=True,
)
