...
proc BusIdle(n:Nat,t:BoolTABLE)=
        sum id:Nat,astat:PAR.(id<n) ->rPAreq(id,astat).DecideIdle(n,t,id,astat) +
     ...
     BusBusy(n:Nat,t,next,destfault:BoolTABLE,busy:Nat)=
       ...
       sum j:Nat.(j<n) -> rPAreq(j,fair).sPAcon(j,lost).BusBusy(n,t,next,destfault,busy) +
     ...
     Resolve2(n:Nat,t:BoolTABLE,next:BoolTABLE)=
        more(next) ->
           (sum j:Nat.(j<n) -> rPDreq(j,End).(get(j,next) -> Resolve2(n,t,invert(j,next)))),
