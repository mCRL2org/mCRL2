...
.. index:: empty;set
   ...
   triple: set;pick;pick

(Finite)  Sets
''''''''''''''

Sets that are guaranteed to be finite can be represented as finite sets.
Finite sets are defined in a similar fashion to lists.
For every sort ``S``, ``FSet(S)`` denotes the sort of finite sets of elements
from ``S``.
...
Note that operators ``<`` and ``<=`` on (finite) sets represent the (strict)
subset operators.

.. list-table:: Predefined operations on finite sets
   :header-rows: 1
   :widths: 15 30 40
   ...
   * - Name
     ...
     - Semantics
   * - :samp:`{a} + {b}`
     - :samp:`FSet({S}) # FSet({S}) -> FSet({S})`
     - Finite set union of :samp:`{a}` and :samp:`{b}`.
   * - :samp:`{a} - {b}`
     - :samp:`FSet({S}) # FSet({S}) -> FSet({S})`
     - Finite set difference of :samp:`{a}` and :samp:`{b}`.
   * - :samp:`{a} * {b}`
     - :samp:`FSet({S}) # FSet({S}) -> FSet({S})`
     - Finite set intersection of :samp:`{a}` and :samp:`{b}`.
   * - :samp:`# {a}`
     - :samp:`FSet({S}) -> Nat`
     - The number of elements in finite set :samp:`{a}`.
   * - :samp:`pick({a})`
     - :samp:`FSet({S}) -> {S}`
     - Return an arbitrary element from finite set  :samp:`{a}`.

Note that ``pick`` allows the definition of additional operations over
(the elements of) finite sets. For example, the maximum ``max_`` of a non-empty
finite set of natural numbers can be defined as follows::

  map max_: FSet(Nat) -> Nat;
  var s: FSet(Nat);
  eqn (s != {}) -> max_(s) = max(pick(s), s - {pick(s)});

Possibly infinite sets can be defined in a similar fashion.
For every sort ``S``, ``Set(S)``denotes the sort of sets of elements from
``S``. Note that ``Set(S)`` is a supertype of ``FSet(S)``. Sets extend the
operations with set comprehension an set complement.
Standard operations on sets are given by the following table.
...
The same operations as for (finite) sets are defined on (finite) bags, with
the difference that bag comprehensions specify the number of times an element
occurs in that bag.
...
One additional operation is defined on bags to count how often an element
occurs in a bag:

.. list-table:: Predefined operations on bags
   :header-rows: 1
   :widths: 15 30 40
...
This defines the type which we designate by ``S`` for brevity, together with
