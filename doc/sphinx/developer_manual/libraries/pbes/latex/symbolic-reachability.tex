...
\begin{align*}
  \sigma_i X_i(d:D) &= \bigwedge\limits_{j \in J_i} \forall e_j : E_j . (\bigvee\limits_{k \in K_j} f_{ijk}(d,e_j)) \implies X_{g_{ij}}(h_{ij}(d, e_j)) \\  
  \sigma_i X_i(d:D) &= \bigwedge\limits_{j \in J_i} \forall e_j : E_j . \bigwedge\limits_{k \in K_j} f_{ijk}(d,e_j) \implies X_{g_{ij}}(h_{ij}(d, e_j)) \\
  \sigma_i X_i(d:D) &= \bigwedge\limits_{j \in J_i}\bigwedge\limits_{k \in K_j} \forall e_j : E_j .  f_{ijk}(d,e_j) \implies X_{g_{ij}}(h_{ij}(d, e_j))
\end{align*}
For the final equation every clause $f_{ijk}(d,e_j))$, for $k \in K_j$, can be treated as its own transition group.
For conjunctive conditions we have to introduce a new equation to ensure that the result in SRF.
\begin{align*}
  \sigma_i X_i(d:D) &= \bigwedge\limits_{j \in J_i} \forall e_j : E_j . (\bigwedge\limits_{k \in K_j} f_{ijk}(d,e_j)) \implies X_{g_{ij}}(h_{ij}(d, e_j)) \\  
  \sigma_i X_i(d:D) &= \bigwedge\limits_{j \in J_i} \forall e_j : E_j . \bigvee\limits_{k \in K_j} \neg f_{ijk}(d,e_j) \lor X_{g_{ij}}(h_{ij}(d, e_j)) \\
  \sigma_i X_i(d:D) &= \bigwedge\limits_{j \in J_i}\bigwedge\limits_{k \in K_j} \forall e_j : E_j .  \textit{true} \implies Y_j(h_{ij}(d, e_j), e_j)
\end{align*}
Where $Y_j$ is a fresh name and its equation defined as follows:
\begin{align*}
  \nu Y_j(d : D, e_j : E_j) &= (\bigvee\limits_{k \in K_j} \neg f_{ijk}(d,e_j) \land Y_j(d, e_j)) \lor (\textit{true} \land X_{g_{ij}}(h_{ij}(d, e_j)))
\end{align*}
Note that this transformations makes the variable $e_j$ visible in the statespace and it is also quantified without a condition.
In practice, this means that the latter transformation can be unwanted in the presence of quantifiers.
...
\attr[n+1]{\alpha}{U, V} & = & \attr[n]{\alpha}{U, V} \\
      & \cup & (T \cap V_\alpha) \\
      & \cup & ((T \cap V_{1-\alpha}) \setminus \bigcup_{i=1}^{n} \pred_i((T \cap V_{1-\alpha}), V \setminus \attr[n]{\alpha}{U,V}) \\
\end{array}
\]
Where $T = \bigcup_{i=1}^{n} \pred_i(V, \attr[n]{\alpha}{U, V})$.
