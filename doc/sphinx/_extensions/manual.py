...
MCRL2_MANUAL_PATH = os.environ.get("MCRL2_MANUAL_PATH", None)
...
def generate_tool_documentation(srcpath, dstpath, binpath, tools):
    for tool in os.listdir(srcpath):
        if tool in tools:
            ...
            man_rst = os.path.join(MCRL2_MANUAL_PATH, "man", tool + ".rst")
            ...
            if os.path.exists(usr_rst):
                with open(usr_rst, "a", encoding="utf-8") as usr_rst_handle:
                    usr_rst_handle.write(f"\n\n.. include:: man/{tool}.rst\n")
            elif os.path.exists(man_rst):
                _LOG.warning(
                    "No help available for {0}. Only man page will be available.".format(
                        tool
                    )
                )
                open(usr_rst, "w+", encoding="utf-8").write(
                    ".. index:: {0}\n\n.. _tool-{0}:\n\n{0}\n{1}\n\n.. include:: man/{0}.rst\n".format(
                        tool, "=" * len(tool)
                    )
                )
            ...
            _LOG.warning(
                "No documentation generated for {0} since it is not part of the build configuration".format(
                    tool
                )
            )
...
def generate_rst(srcpath, binpath, tools):
    ...
    generate_tool_documentation(
        os.path.join(srcpath, "tools", "release"),
        os.path.join(dstdir, "release"),
        binpath,
        tools,
    )
    generate_tool_documentation(
        os.path.join(srcpath, "tools", "experimental"),
        os.path.join(dstdir, "experimental"),
        binpath,
        tools,
    )
