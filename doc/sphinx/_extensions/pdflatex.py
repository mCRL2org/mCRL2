...
_ARTICLEINDEXTEMPLATE = string.Template(
    open(
        os.path.join(_PWD, "libraries_templates", "articles.rst.template"),
        encoding="utf-8",
    ).read()
)
...
def makepdf(src):
    ...
    try:
        title = re.search(
            r"\\title{(.*?)}", open(src, encoding="utf-8").read(), re.DOTALL
        )
        ...
        for f in os.listdir(srcdir):
            ...
            if f.endswith(".tex"):
                ...
                path = os.path.relpath(
                    os.path.join(srcdir, basename), os.path.join(_PWD, "..")
                )

                # Turns out that sphinx only likes forward slashes
                log_nonl(f"Compiling pdf {texdir}")
                titles.append(
                    ":download:`{0} </{1}.pdf>`".format(
                        makepdf(texdir), path.replace("\\", "/")
                    )
                )

        # Write all the titles into the articles.rst
        open(os.path.join(dstdir, "articles.rst"), "w+", encoding="utf-8").write(
