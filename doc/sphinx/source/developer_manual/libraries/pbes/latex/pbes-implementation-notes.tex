...
%TCIDATA{LastRevised=Thursday, February 21, 2013 11:13:43}
...
\begin{definition}
Let $\varphi $ be a predicate formula in which every predicate variable
occurs at most once. Provided that $X\in occ(\varphi )$, the guard of $X$ in 
$\varphi $, denoted $guard^{X}(\varphi )$, is defined inductively as follows:
\end{definition}
...
guard^{X}(c) & = & false \\ 
guard^{X}(Y) & = & true \\ 
guard^{X}(\forall d:D.\varphi ) & = & guard^{X}(\varphi ) \\ 
guard^{X}(\exists d:D.\varphi ) & = & guard^{X}(\varphi ) \\ 
guard^{X}(\varphi \wedge \psi ) & = & \left\{ 
\begin{array}{lll}
s(\varphi )\wedge guard^{X}(\psi ) &  & \text{if }X\in occ(\psi ) \\ 
s(\psi )\wedge guard^{X}(\varphi ) &  & \text{if }X\in occ(\varphi )%
\end{array}%
\right. \\ 
guard^{X}(\varphi \vee \psi ) & = & \left\{ 
\begin{array}{lll}
n(\varphi )\wedge guard^{X}(\psi ) &  & \text{if }X\in occ(\psi ) \\ 
ns(\psi )\wedge guard^{X}(\varphi ) &  & \text{if }X\in occ(\varphi ),%
\end{array}%
\right.%
\end{array}%
\end{equation*}%
...
s(\varphi ) & = & \left\{ 
\begin{array}{lll}
\varphi &  & \text{if }\varphi \text{ is a simple formula} \\ 
true &  & \text{otherwise}%
\end{array}%
\right. \\ 
ns(\varphi ) & = & \left\{ 
\begin{array}{lll}
\lnot \varphi &  & \text{if }\varphi \text{ is a simple formula} \\ 
true &  & \text{otherwise}%
\end{array}%
\right.%
\end{array}%
\end{equation*}%
Let $X(d:D)=\varphi $ be a PBES equation with $d=[d_{1},\ldots ,d_{N_{X}}]$,
and let $Y(e)\in occ(\varphi )$. Let $source(Y(e),X)$ be a substitution $%
[d_{j_{1}}:=f_{1},\ldots ,d_{j_{jm}}:=f_{p}]$, such that $%
guard^{Y(e)}\Rightarrow \bigwedge_{k=1\ldots p}d_{j_{k}}=f_{k}$.

\textbf{Step 1} of the stategraph algorithm is to apply the transformation $%
Y(e)\mapsto \sigma (Y(e))$ with $\sigma =source(Y(e),X)$ to all predicate
variables $Y(e)$ appearing in one of the right hand sides of a PBES equation.
...
then }}V(Y,i):=e_{i}$ \\ 
$\qquad \qquad \qquad \qquad \text{\textbf{else }}cf(Y,i):=(V(Y,i)=e_{i})$
\\ 
...
$\qquad \qquad \qquad \qquad \qquad \text{\textbf{if }}cf(Y,i)\text{ \textbf{%
then }}cf(Y,i):=false$; $todo:=todo\cup \{Y\}\qquad \qquad \qquad \qquad
\qquad $ \\ 
$\qquad \qquad \qquad \qquad \text{\textbf{else if }}e_{i}\in
par(X_{k})\wedge X_{k}=Y\wedge pos(X_{k},e_{i})\neq i$ \\ 
$\qquad \qquad \qquad \qquad \qquad \text{\textbf{if }}cf(Y,i)\text{ \textbf{%
then }}cf(Y,i):=false$; $todo:=todo\cup \{Y\}$ \\ 
$\qquad \qquad \qquad \qquad \text{\textbf{else if }}e_{i}\in
...
\begin{equation*}
\begin{tabular}{l}
\textsc{ComputeControlflowGraph(}$\mathcal{E}$, $X_{init}(e_{init})$\textsc{)%
} \\ 
$u_{0}:=\overline{X_{init}(e_{init})}$ \\ 
$V:=\{u_{0}\}$ \\ 
$E:=\emptyset $ \\ 
$todo:=\{u_{0}\}$ \\ 
$guards(u_{0}):=\emptyset $ \\ 
\textbf{while} $todo\neq \emptyset $ \textbf{do} \\ 
$\qquad $\textbf{choose} $u=X_{k}(e_{k})\in todo$ \\ 
$\qquad todo:=todo\setminus \{u\}$ \\ 
$\qquad \text{\textbf{if }}occ(\varphi _{k})=\emptyset \text{ \textbf{then }}%
guards(u):=guards(u)\cup \{guard^{X_{k}}(\varphi _{k})\}$ \\ 
$\qquad \text{\textbf{for }}Y(e)\in occ(\varphi _{k})\text{ \textbf{do}}$ \\ 
$\qquad \qquad g:=R(guard^{Y(e)}(\varphi _{k}),[d_{k}:=e_{k}])$ \\ 
$\qquad \qquad \text{\textbf{if }}g\neq false$ \\ 
$\qquad \qquad \qquad v:=\overline{Y(e)[d_{k}:=e_{k}]}$ \\ 
$\qquad \qquad \qquad \text{\textbf{if }}v\notin V$ \\ 
$\qquad \qquad \qquad \qquad V:=V\cup \{v\}$ \\ 
$\qquad \qquad \qquad \qquad guards(v):=\emptyset $ \\ 
$\qquad \qquad \qquad \qquad todo:=todo\cup \{v\}$ \\ 
$\qquad \qquad \qquad guards(u):=guards(u)\cup \{g\}$ \\ 
