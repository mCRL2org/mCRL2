...
%TCIDATA{LastRevised=Monday, March 04, 2013 12:37:58}
...
\mathrm{npred}(\varphi )$. The functions $\mathrm{pred}(\varphi )$, $\mathrm{%
data}(\varphi ,i)$ and $\mathrm{PVI}(\varphi ,i)$ are such that the
...
pred}(\varphi )(\mathrm{data}(\varphi ,i))$. Let $\psi $ be a predicate
...
\begin{array}{lll}
e &  & \text{if }guard^{i}(\mathrm{PVI}(\varphi ,i))\Rightarrow d[j]\approx e
\\ 
\bot &  & \text{otherwise}%
\end{array}%
\right.
...
This projection applied to the predicate variable $X_{k}(e_{1},\cdots ,e_{m})
$ is denoted as $\overline{X_{k}(e_{1},\cdots ,e_{m})}$. The mapping $R$ is
a rewriter that takes a substitution to data variables as second argument.
...
$sig(u_{0}):=\emptyset $ \\ 
\textbf{while} $todo\neq \emptyset $ \textbf{do} \\ 
$\qquad $\textbf{choose} $u=X(e)\in todo$ \\ 
$\qquad todo:=todo\setminus \{u\}$ \\ 
$\qquad sig(u):=sig(R(\varphi _{X},[d_{X}:=e])$ \\ 
$\qquad \text{\textbf{for }}Y(e)\in occ(\varphi _{X})\text{ \textbf{do}}$ \\ 
$\qquad \qquad g:=R(guard^{Y(e)}(\varphi _{X}),[d_{X}:=e])$ \\ 
$\qquad \qquad \text{\textbf{if }}g\neq false$ \\ 
$\qquad \qquad \qquad v:=\overline{Y(e)[d_{X}:=e]}$ \\ 
...
$\qquad todo:=todo\setminus \{u\}$ \\ 
$\qquad sig(u):=sig(R(\varphi _{X},[d_{X}:=e])$ \\ 
$\qquad \text{\textbf{for }}Y(f)\in occ(\varphi _{X})\text{ \textbf{do}}$ \\ 
$\qquad \qquad $\textbf{let} $(X,p)\longrightarrow _{must}(Y,q)$ \\ 
$\qquad \qquad g:=R(guard^{Y(f)}(\varphi _{X}),[d_{X}[p]:=e])$ \\ 
$\qquad \qquad \text{\textbf{if }}g\neq false\wedge ((X\neq Y)\vee
(d_{Y}[q]\neq f[q]\wedge source(X,i,q)=\bot ))$ \\ 
$\qquad \qquad \qquad v:=Y((f[d_{X}:=e])[q])$ \\ 
$\qquad \qquad \qquad \text{\textbf{if }}v\notin V$ \\ 
$\qquad \qquad \qquad \qquad V:=V\cup \{v\}$ \\ 
$\qquad \qquad \qquad \qquad todo:=todo\cup \{v\}$ \\ 
$\qquad \qquad \qquad E:=E\cup \{(u,v)\}$ \\ 
