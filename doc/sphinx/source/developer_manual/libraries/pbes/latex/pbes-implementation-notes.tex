...
%TCIDATA{LastRevised=Tuesday, March 12, 2013 17:52:37}
...
$ \\ 
$\qquad \qquad \qquad \qquad \text{\textbf{for }}j$ such that $%
(X,d_{X}[j])\in B_{k}\wedge d_{X}[j]\notin marking(u)$ \\ 
$\qquad \qquad \qquad \qquad \qquad \qquad \wedge (Y,d_{Y}[l])\notin
B_{k}\wedge d_{X}[j]\in FV(f[l])$ \\ 
$\qquad \qquad \qquad \qquad \qquad \text{\textbf{for }}k^{\prime }$ such
that $(Y,d_{Y}[l])\in B_{k^{\prime }}\text{ \textbf{do}}$ \\ 
$\qquad \qquad \qquad \qquad \qquad \qquad \text{\textbf{if }}\exists
w=Y(g)\in V_{k^{\prime }}$ \textbf{such that} ??? $\in marking(w)$ \\ 
