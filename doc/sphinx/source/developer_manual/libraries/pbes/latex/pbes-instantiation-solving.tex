...
$}\\
\qquad \colorbox{lightgray}{$V := V \setminus \textsc{Attr}(todo, 1)$}\\
\colorbox{lightgray}{$
\textbf{else if } todo \neq \emptyset \land X_{init}(e_{init}) \in S_1 \textbf{ then }
$}\\
\qquad \colorbox{lightgray}{$V := V \setminus \textsc{Attr}(todo, 0)$}\\
\textbf{return }(V, E),
\end{array}
\end{equation*}
...
\qquad discovered:=discovered\ \cup \mathrm{occ}(\psi ^{e}) \\
\qquad \colorbox{lightgray}{$todo, irrelevant := \textsc{PruneTodo}(init, todo, irrelevant) $}\\
\textbf{if } (\colorbox{lightgray}{$todo \cup irrelevant$}) \neq \emptyset \land X_{init}(e_{init}) \in S_0 \textbf{ then }
\\
\qquad V := V \setminus \textsc{Attr}(\colorbox{lightgray}{$todo \cup irrelevant$}, 1)\\
\textbf{else if } (\colorbox{lightgray}{$todo \cup irrelevant$}) \neq \emptyset \land X_{init}(e_{init}) \in S_1 \textbf{ then } \\
\qquad V := V \setminus \textsc{Attr}(\colorbox{lightgray}{$todo \cup irrelevant$}, 0)\\
\textbf{return }(V, E),
\end{array}
\end{equation*}
