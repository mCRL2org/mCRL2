...
init:=R(X_{init}(e_{init})) \\
todo:=\{init\} \\
\colorbox{lightgray}{$removed := \emptyset$} \\
discovered:=\{init\} \\
(V,E) :=(\emptyset ,\emptyset ) \\ 
S_0:= \emptyset \\
S_1:= \emptyset \\
...
\qquad todo:=todo\cup (\mathrm{occ}(\psi ^{e})\setminus discovered) \\
\qquad discovered:=discovered\ \cup \mathrm{occ}(\psi ^{e}) \\
\qquad \colorbox{lightgray}{$todo, removed := \textsc{PruneTodo}(init, todo, removed) $}\\
\colorbox{lightgray}{$
\textbf{if } todo \neq \emptyset \land X_{init}(e_{init}) \in S_0 \textbf{ then }
$}\\
\qquad \colorbox{lightgray}{$V := V \setminus \textsc{Attr}(todo \cup removed, 1)$}\\
\colorbox{lightgray}{$
\textbf{else if } todo \neq \emptyset \land X_{init}(e_{init}) \in S_1 \textbf{ then }
$}\\
\qquad \colorbox{lightgray}{$V := V \setminus \textsc{Attr}(todo \cup removed, 0)$}\\
\textbf{return }(V, E),
\end{array}
\end{equation*}
...
\begin{equation*}
\begin{array}{l}
\textsc{PruneTodo}(init, todo, removed) \\ 
todo' := \{init\} \\
done' := \{init\} \\
new\_todo := \emptyset \\
...
\qquad \If d(u) = \bot \wedge succ(u) = \emptyset \Space \Then \\
\qquad \qquad new\_todo := new\_todo \cup \{ u \} \\
new\_todo := new\_todo \cap (todo \cup removed) \\
new\_removed := (todo \cup removed) \setminus new\_todo \\
