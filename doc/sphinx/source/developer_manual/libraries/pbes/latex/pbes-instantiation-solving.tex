...
\qquad \If d(u) = \alpha \vee succ(u)\subseteq A \Space \Then \\ 
...
\qquad \If (d(u) = \alpha \wedge succ(u) \cap A \neq \emptyset)
           \vee (d(u) \neq \alpha \wedge succ(u) \subseteq A \cup S) \Space \Then \\ 
\qquad \qquad A := A \cup \{ u \} \\ 
...
A third version of an attractor set computation is \textsc{AttrSimple}. It is used in optimization 3.
...
\text{)} \\ 
\If W_{1-\alpha }^{\prime }= \emptyset \Space \Then \\ 
...
\colorbox{lightgray}{$
\If W_{1-\alpha }^{\prime }=B \Space \Then $}\\ 
\colorbox{lightgray}{$
...
U:=\{v\in V\mid r(v)=m\} \\ 
\colorbox{lightgray}{$
\If h=m\wedge even(m) \Space \Then \Return (\emptyset,V) $}\\ 
\colorbox{lightgray}{$
\If h=m\wedge odd(m) \Space \Then \Return (V,\emptyset) $}\\
A:=\text{\textsc{AttrDefault}}\left( U,\alpha \right)  \\ 
W_{0}^{\prime },W_{1}^{\prime }:=\text{\textsc{SolveRecursive(}}V\setminus A%
\text{)} \\ 
\If W_{1-\alpha }^{\prime }= \emptyset \Space \Then \\ 
...
\begin{equation*}
\begin{array}{l}
\text{\textsc{AttrDefault(}}A,\alpha \text{\textsc{)}} \\ 
todo:=\bigcup\limits_{u\in A}\left( pred(u)\setminus A\right)  \\ 
\While todo\neq \emptyset \Space \Do \\ 
\qquad \Choose u\in todo \\ 
\qquad todo:=todo\ \backslash \ \{u\} \\ 
...
\end{array}%
\end{equation*}%
Note that the mapping $\tau$ is a global variable of \textsc{AttrDefault}, which is ugly.
...
\qquad \psi ^{e}:=R(\varphi _{k}[d_{k}:=e]) \\ 
\qquad \colorbox{lightgray}{$%
\psi^e := \Space \If \sigma_k = \mu \Space \Then R(\psi^e[X^e := false]) \Space \Else  R(\psi^e[X^e := true]) $}\\
...
\qquad todo:=todo\ \backslash \ \{X_{k}(e)\} \\ 
\qquad \psi ^{e}:=R(\varphi _{k}[d_{k}:=e]) \\ 
\qquad \psi^e := \Space \If \sigma_k = \mu \Space \Then R(\psi^e[X^e := false])
...
Note that this version of the algorithm is not available in the tool \textbf{pbessolve}.
...
\[
\begin{array}{l}
\If b = true \Space \Then S_0 := S_0 \cup \{ X^e \} \\
\If b = false \Space \Then S_1 := S_1 \cup \{ X^e \} \\
\end{array}
\]
...
This algorithm is the default in \textbf{pbessolve}.
...
\qquad \If b = true \Space \Then S_0 := S_0 \cup \{ X^e \} \\
\qquad \If b = false \Space \Then S_1 := S_1 \cup \{ X^e \} \\
\qquad (V,E) := (V,E) \cup SG^{0}(X^{e},\psi ^{e}) \\ 
...
\colorbox{lightgray}{$V := V \setminus \textsc{AttrSimple}(irrelevant)$} \\
\Return(V, E),
\end{array}
\end{equation*}
...
\colorbox{lightgray}{$
\qquad \If X_{init}(e_{init}) \in S_0  \Space \Then V := V \setminus \textsc{AttrDefault}(todo, 1)
$}\\
\colorbox{lightgray}{$
\qquad \Else \If X_{init}(e_{init}) \in S_1 \Space \Then V := V \setminus \textsc{AttrDefault}(todo, 0)$}
\\
\Return(V, E),
\end{array}
\end{equation*}
...
computations are expensive operations, so in \textbf{pbessolve} the function $apply\_attractor$
...
% \begin{equation*}
% \begin{array}{l}
% \qquad \If b = true \Space \Then S_0 := \colorbox{lightgray}{$
% \textsc{AttrCheap}(S_0, X^e, 0)$} \\
% \qquad \If b = false \Space \Then S_1 := \colorbox{lightgray}{$
...
%\If w \in U \Space \Then \\
%\qquad \text{visited}(w) := false\\
%\qquad \If (w \in \mathsf{dom}(d) \Rightarrow d(w) = \textsf{parity}(p)) \Space \Then \\
...
\text{visited} := [] \\
b_0,b_1 := false,false \\
\textbf{for } u \in done \cap \textsf{dom}(r) \Space \Do \\
\qquad \If u \notin \textsf{keys}(\text{visited}) \Space \Then \text{visited}(u) := false\\
\qquad b := \textsc{FindLoop}(done, u, u, r(u), \text{visited}) \\
\qquad \text{visited}(u) := b\\
\qquad \If b \Space \Then \\
\qquad \qquad \If r(u) \bmod 2 = 0 \Space \Then S_0,b_0 := S_0 \cup \{u\}, true\\
\qquad\qquad \textbf{else } S_1,b_1 := S_1 \cup \{u\}, true\\
\If b_0 \Space \Then S_0 := \textsc{AttrDefault}(S_0, 0)\\ 
\If b_1 \Space \Then S_1 := \textsc{AttrDefault}(S_1, 1)\\ 
\Return S_0, S_1
\end{array}
\end{equation*}
The above optimisation can be integrated in the algorithm as follows. In \textbf{pbessolve}
this optimisation is implemented as an extension of \textsc{PbesinstStructureGraph$_2$}.
...
\If todo \neq \emptyset \land X_{init}(e_{init}) \in S_0 \Space \Then \\
\qquad V := V \setminus \textsc{AttrDefault}(todo, 1) \\
\Else \If todo \neq \emptyset \land X_{init}(e_{init}) \in S_1 \Space \Then \\
\qquad V := V \setminus \textsc{AttrDefault}(todo, 0) \\
\Return (V, E)
\end{array}%
\end{equation*}%
...
\Return \textsc{AttrDefault}(S_0, 0), \textsc{AttrDefault}(S_1, 1)
\end{array}
\end{equation*}
...
\qquad discovered:=discovered\ \cup \mathrm{occ}(\psi ^{e}) \\
\qquad \colorbox{lightgray}{$%
S_0, S_1 := \text{\textsc{FatalAttractors}}(V, S_0, S_1) $} \text{(executed periodically)} \\
\If todo \neq \emptyset \land X_{init}(e_{init}) \in S_0 \Space \Then \\
\qquad V := V \setminus \textsc{AttrDefault}(todo, 1) \\
\Else \If todo \neq \emptyset \land X_{init}(e_{init}) \in S_1 \Space \Then \\
\qquad V := V \setminus \textsc{AttrDefault}(todo, 0) \\
...
In optimisation 7 the sets $S_0$ and $S_1$ are extended by solving a partial game. In \textbf{pbessolve} this optimisation is implemented as an extension of \textsc{PbesinstStructureGraph$_2$}.
...
\qquad discovered:=discovered\ \cup \mathrm{occ}(\psi ^{e}) \\
\qquad \colorbox{lightgray}{$
S_0, S_1 := \textsc{PartialSolve}(V,todo,S_0,S_1) $} \text{(executed periodically)} \\
\If todo \neq \emptyset \land X_{init}(e_{init}) \in S_0 \Space \Then \\
\qquad V := V \setminus \textsc{AttrDefault}(todo, 1) \\
\Else \If todo \neq \emptyset \land X_{init}(e_{init}) \in S_1 \Space \Then \\
\qquad V := V \setminus \textsc{AttrDefault}(todo, 0) \\
