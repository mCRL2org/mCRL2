...
\qquad todo' := todo' \setminus \{ u \} \\
\qquad done' := done' \cup \{ u \} \\
\qquad \If d(u) = \blacktriangle \wedge succ(u) \cap S_1 = \emptyset \Then \\
\qquad \qquad todo' := todo' \cup (succ(u) \setminus (S_0 \cup done')) \\
\qquad \If d(u) = \blacktriangledown \wedge succ(u) \cap S_0 = \emptyset \Then \\
\qquad \qquad todo' := todo' \cup (succ(u) \setminus (S_1 \cup done')) \\
\qquad \If d(u) = \bot \wedge succ(u) \setminus (S_0 \cup S_1 \cup done') = \emptyset \Then \\
\qquad \qquad todo' := todo' \cup succ(u) \\
\qquad \If d(u) = \bot \wedge succ(u) = \emptyset \Then \\
...
There is a second, more advanced version available:

\begin{equation*}
\begin{array}{l}
\textsc{PbesinstStructureGraphPruneTodoAlternative}(\mathcal{E}, X_{init}(e_{init}), R) \\ 
init:=R(X_{init}(e_{init})) \\
todo:=\{init\} \\
discovered:=\{init\} \\
(V,E) :=(\emptyset ,\emptyset ) \\ 
...
\qquad \colorbox{lightgray}{$V' := \textsc{PruneTodoAlternative}(init, S_0, S_1) $}\\
...
where \textsc{PruneTodoAlternative} is defined as

\begin{equation*}
\begin{array}{l}
\textsc{PruneTodoAlternative}(init, todo, irrelevant) \\ 
todo' := \{ init \} \\
done' := \{ init \} \\
new\_todo := \emptyset \\
\While todo' \neq \emptyset \Do \\
...
\qquad todo' := todo' \setminus \{ u \} \\
\qquad done' := done' \cup \{ u \} \\
\qquad \colorbox{lightgray}{$ \If u \in dom(\tau) \land \tau(u) \notin done' \Then $}\\
\qquad \qquad \colorbox{lightgray}{$todo' := todo' \cup \{ \tau(u) \} $}\\
\qquad \Else \\
\qquad \qquad \If d(u) = \blacktriangle \wedge succ(u) \cap S_1 = \emptyset \Then \\
\qquad \qquad \qquad todo' := todo' \cup (succ(u) \setminus (S_0 \cup done')) \\
\qquad \qquad \If d(u) = \blacktriangledown \wedge succ(u) \cap S_0 = \emptyset \Then \\
\qquad \qquad \qquad todo' := todo' \cup (succ(u) \setminus (S_1 \cup done')) \\
\qquad \qquad \If d(u) = \bot \wedge succ(u) \setminus (S_0 \cup S_1 \cup done') = \emptyset \Then \\
\qquad \qquad \qquad todo' := todo' \cup succ(u) \\
\qquad \qquad \If d(u) = \bot \wedge succ(u) = \emptyset \Then \\
\qquad \qquad \qquad new\_todo := new\_todo \cup \{ u \} \\
new\_todo := new\_todo \cap (todo \cup irrelevant) \\
new\_irrelevant := (todo \cup irrelevant) \setminus new\_todo \\
\Return new\_todo, new\_irrelevant
\end{array}
...
\qquad \colorbox{Apricot}{$ \For v \in (U_j \cap X) \setminus S_{\alpha} \Do $} \\
\qquad \colorbox{Apricot}{$ \qquad \If (\alpha = 0 \land d(u) = \blacktriangledown) \lor (\alpha = 1 \land d(u) = \blacktriangle) \Then $} \\
\qquad \colorbox{Apricot}{$ \qquad \qquad \tau(v) := w \textbf{ with } w \in succ(v) \cap Y $} \\
...
\qquad \qquad \qquad \colorbox{Apricot}{$ \For v \in U_j \setminus S_{\alpha} \Do $} \\
\qquad \qquad \qquad \colorbox{Apricot}{$ \qquad \If (\alpha = 0 \land d(u) = \blacktriangledown) \lor (\alpha = 1 \land d(u) = \blacktriangle) \Then $} \\
\qquad \qquad \qquad \colorbox{Apricot}{$ \qquad \qquad \tau(v) := w \textbf{ with } w \in succ(v) \cap Y $} \\
