...
\subsubsection{The functions source, dest and copy}
...
\subsubsection{Global control flow graph (2)}
...
$V:=\emptyset $ \\ 
$E:=\emptyset $ \\ 
$todo:=\{(X_{init},project_{X}(e_{init}))\}$ \\ 
\textbf{while} $todo\neq \emptyset $ \textbf{do} \\ 
...
$\qquad q:=$ $\mathrm{target}(X,i,\downarrow _{X}l)$ \\ 
$\qquad \text{\textbf{if }}q=\bot $ \textbf{then} \\ 
$\qquad \qquad \text{\textbf{choose }}k$ \textbf{such that }$\text{\textsc{%
IsRelated(}}Y,\downarrow _{Y}l,X,\downarrow _{X}k\text{\textsc{)}}$ \\ 
