...
A fairly consistent argument model
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

With ``mcrl2-gui`` in place, it is important that command-line options are also
accessible from this graphical user interface. The choice has been made to
provide ``mcrl2-gui`` with an XML description of the arguments a tool takes.
This XML is generated by the tool itself. Currently, the XML is not complete in
the sense that some information is missing (such as the category to which a tool
belongs), and has a confusing structure in other places. We attempt to define
the command-line options in such a way that the command-line library and the
``mcrl2-gui`` tool use the same structure:
  ...
  <tool name="string" category="string">
    ...
    <arguments>
      <argument
          ...
          [ absentvalue="string" ]
          [ filemask="string" ]
          [ repeat="increment|append|overwrite" ]
          [ optional ]
      >
        <description>text</description>
        [ <option name="string">text</option> ]*
      </argument>
    </arguments>
  </tool>

This covers both named and positional arguments.
We assume *every* option is given a value on the commandline either using the 
long syntax, the short syntax or the positional syntax as follows::

  toolname --argument=value -a v pos1value

The ``optiontype`` can be either of ``int,string,infile,outfile,enum,bool``. If
unspecified, it defaults to ``bool``. If it is chosen to be ``enum``, then the
``option`` entities represent the possible values it can take, and ``enumlabel``
specifies the name of the argument's value (which defaults to ``VALUE`` if left
unspecified).

The ``absentvalue`` is the value that this argument takes if it is not specified
on the command line. If unspecified, then the user is required to at include the
argument on the commandline. If specified, then the argument is optional, i.e.,
the following two command lines are equivalent for an argument ``arg`` with 
``absentvalue=5``::

  $ toolname --arg=5
  $ toolname

The ``defaultvalue`` is the value that this argument takes if the argument 
appears on the command line, but isn't assigned a value. The following command 
lines are therefore equivalent for an argument ``arg`` with ``defaultvalue=5``::
...
**verbose**
  XML::
    ...
    <argument name="verbose" shortname="v" absentvalue="false" defaultvalue="true">
      <description>Print more verbose output.</description>
    </argument>
...
**lps2lts bithashing**
  XML::
    ...
    <argument name="bithashing" type="int" shortname="b" defaultvalue="10000" valuelabel="SIZE" optional>
      <description>Use bithashing table with SIZE entries.</description>
    </argument>

  Usage::

    $ toolname -b
    sets bithashing=10000
    $ toolname
    bithashing remains undefined
    $ toolname --bithashing=300
    sets bithashing=300

**lps2pbes formula file**
  XML::

    <argument name="formulafile" type="infile" shortname="f">
      <description>File containing a mu-calculus formula.</description> 
    </argument>

  Usage::

    $ toolname -f bla.mcf
    sets formulafile="bla.mcf", checks if it exists
    $ toolname
    not allowed: argument is not optional.

**output file positional argument**
  XML::

    <argument name="outputfile" type="outfile" position="2" optional>
      <description>File to write to.
    </argument>

  Usage::

    $ toolname infile outfile
    sets outputfile="outfile"blocking time is in fact 
    $ toolname infile
    sets outputfile=undefined

**ltsconvert equivalence**
  XML::

    <argument name="equivalence" shortname="e" type="enum" absentvalue="bisim">
      <description>The equivalence to use for minimisation. Choose from:</description>
      <option name="bisim">Strong bisimulation (default)</option>
      <option name="sim">Simulation</option>
    </argument>

  Usage::

    $ toolname -e sim
    sets equivalence="sim"
    $ toolname -e
    not allowed: argument has no default value.
    $ toolname
    sets equivalence="bisim"

To ease scripting, it is assumed that if an argument occurs more than once, the
last occurrence counts::

  $ toolname -v --verbose=false
  sets verbose=false

Incremented arguments are not supported (e.g. the syntax "-vvv" instead of
"-v 3").
