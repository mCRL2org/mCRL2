...
The mCRL2 toolset is a collection of tools to model and analyse the behaviour of 
communicating processes. This tutorial shows by example how to use the toolset.
*mcrl2-gui* is used to apply the tools to files. Note that it is not strictly
necessary to use *mcrl2-gui*. All tools can also be used from the command line.
Use the ``--help`` flag to view the list of available options of a tool, or
browse the :ref:`tool manual pages <man pages>`.
