...
class TestRunner(testing.TestRunner):
    def __init__(self):
        ...
        self.tests = [ AlphabetTest('alphabet_{}'.format(i)                           , self.settings) for i in I ] + \
                     ...
                     [ LtscompareTest('ltscompare_{}'.format(i), 'bisim'              , self.settings) for i in I ] + \
                     [ LtscompareTest('ltscompare_{}'.format(i), 'branching-bisim'    , self.settings) for i in I ] + \
                     [ LtscompareTest('ltscompare_{}'.format(i), 'dpbranching-bisim'  , self.settings) for i in I ] + \
                     [ LtscompareTest('ltscompare_{}'.format(i), 'weak-bisim'         , self.settings) for i in I ] + \
                     [ LtscompareTest('ltscompare_{}'.format(i), 'dpweak-bisim'       , self.settings) for i in I ] + \
                     [ LtscompareTest('ltscompare_{}'.format(i), 'sim'                , self.settings) for i in I ] + \
                     [ LtscompareTest('ltscompare_{}'.format(i), 'trace'              , self.settings) for i in I ] + \
                     [ LtscompareTest('ltscompare_{}'.format(i), 'weak-trace'         , self.settings) for i in I ] + \
                     [ BisimulationTest('bisimulation_{}'.format(i), 'bisim'          , self.settings) for i in I ] + \
                     [ BisimulationTest('bisimulation_{}'.format(i), 'branching-bisim', self.settings) for i in I ] + \
                     [ BisimulationTest('bisimulation_{}'.format(i), 'weak-bisim'     , self.settings) for i in I ] + \
