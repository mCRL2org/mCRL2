...
class TestRunner(testing.TestRunner):
    def __init__(self):
        ...
        self.tests = [ AlphabetTest('alphabet_{}'.format(i), self.settings)                                            for i in I ] + \
                     ...
                     [ LtscompareTest('ltscompare_bisim_{}'.format(i)            , 'bisim'            , self.settings) for i in I ] + \
                     [ LtscompareTest('ltscompare_branching_bisim_{}'.format(i)  , 'branching-bisim'  , self.settings) for i in I ] + \
                     [ LtscompareTest('ltscompare_dpbranching_bisim_{}'.format(i), 'dpbranching-bisim', self.settings) for i in I ] + \
                     [ LtscompareTest('ltscompare_weak_bisim_{}'.format(i)       , 'weak-bisim'       , self.settings) for i in I ] + \
                     [ LtscompareTest('ltscompare_dpweak_bisim_{}'.format(i)     , 'dpweak-bisim'     , self.settings) for i in I ] + \
                     [ LtscompareTest('ltscompare_sim_{}'.format(i)              , 'sim'              , self.settings) for i in I ] + \
                     [ LtscompareTest('ltscompare_trace_{}'.format(i)            , 'trace'            , self.settings) for i in I ] + \
                     [ LtscompareTest('ltscompare_weak_trace_{}'.format(i)       , 'weak-trace'       , self.settings) for i in I ] + \
